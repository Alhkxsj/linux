...
static int hci_read_sync_train_params_sync(struct hci_dev *hdev)
{
	...
	ret = __hci_cmd_sync_status(hdev, HCI_OP_READ_SYNC_TRAIN_PARAMS,
				    0, NULL, HCI_CMD_TIMEOUT);
	/*
	 * PEARL: the MT6631 (connac gen2) firmware reports the Synchronization
	 * Train feature bit but rejects this command with an error status,
	 * aborting the whole init sequence. The command is optional - log and
	 * carry on.
	 */
	if (ret < 0) {
