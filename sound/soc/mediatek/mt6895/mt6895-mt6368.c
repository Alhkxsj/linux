...
/*
 * The codec input muxes (PGA_L_Mux/ADC_L_Mux/MISO*_MUX) are DAPM mux widgets:
 * their kcontrols only come into existence inside snd_soc_dapm_new_widgets(),
 * which soc-core.c runs *after* late_probe. Applying the defaults from
 * late_probe therefore always failed with "kcontrol ... missing" and left the
 * capture path disconnected (PGA_L_Mux=None, ADC_L_Mux=Idle) so the whole
 * chain stayed unpowered and capture returned pure zeros. fixup_controls runs
 * right after snd_soc_dapm_new_widgets() and before snd_card_register(), which
 * is the first point where these kcontrols can be looked up.
 */
static void mt6895_mt6368_fixup_controls(struct snd_soc_card *card)
{
	int i;
...
static struct snd_soc_card mt6895_mt6368_soc_card = {
	/*
	 * name  -> snd_card shortname, also the debugfs/procfs directory name.
	 *          Keep the driver-ish id here so /sys/kernel/debug/asoc paths
	 *          and any userspace matching stay stable.
	 * long_name / components carry the human readable hardware description
	 *          that userspace (PipeWire, /proc/asound/cardN) shows.
	 */
	.name = "mt6895-mt6368",
	.long_name = "MediaTek MT6895 Audio + NXP TFA9874 Smart Amplifier",
	.driver_name = "mt6895-mt6368",
	.components = "codec:MediaTek-MT6895-Audio ampl:NXP-TFA9874-Smart-Amplifier",
	.owner = THIS_MODULE,
	...
	.fixup_controls = mt6895_mt6368_fixup_controls,
};
