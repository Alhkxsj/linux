...
struct mt63xx_accdet_data {
	...
	/*
	 * Separate jack for the headset microphone. One jack cannot tell a
	 * 3-pole headphone from a 4-pole headset in userspace: its kcontrol is
	 * a single boolean, so "Headphone Jack" goes high for both. UCM and the
	 * ALSA card profiles need a distinct "Headset Mic Jack" to decide
	 * whether an input port exists.
	 */
...
static void send_status_event(u32 cable_type, u32 status)
{
	...
	case HEADSET_NO_MIC:
		...
		if (status == 0) {
			...
			snd_soc_jack_report(&accdet->jack, report,
					SND_JACK_MICROPHONE);
			snd_soc_jack_report(&accdet->mic_jack, report,
					SND_JACK_MICROPHONE);
		}
	...
	case HEADSET_MIC:
		...
		snd_soc_jack_report(&accdet->mic_jack, report,
				SND_JACK_MICROPHONE);
		/*
		 * A 4-pole headset carries headphones as well as a mic, so
		 * report both bits: SND_JACK_HEADSET is HEADPHONE|MICROPHONE.
		 * The vendor code only reported MICROPHONE here and relied on
		 * the Android HAL reading its own cable_type; generic userspace
		 * keys off SW_HEADPHONE_INSERT / the headphone jack kcontrol and
		 * would see no output device at all.
		 */
		snd_soc_jack_report(&accdet->jack,
				status ? SND_JACK_HEADPHONE : 0,
				SND_JACK_HEADPHONE);
			...
			struct snd_soc_card *card)
{
	...
	/*
	 * Named "Headphone" so the kcontrol comes out as "Headphone Jack",
	 * which is what the ALSA card-profile / UCM jack bindings look for.
	 * The vendor name "Headset Jack" matches nothing in generic userspace,
	 * so KDE/PipeWire never learned the port existed.
	 */
	ret = snd_soc_card_jack_new(card,
				    "Headphone",
				    SND_JACK_HEADSET |
				    ...
				    &accdet->jack);
	if (ret)
		pr_err("Headphone Jack create failed\n");

	ret = snd_soc_card_jack_new(card, "Headset Mic", SND_JACK_MICROPHONE,
				    &accdet->mic_jack);
	if (ret) {
