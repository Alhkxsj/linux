...
/*
 * XAGA bring-up diagnostics (see probe): delayed MODE readback so the
 * value lands inside the expdb console-record window.
 */
static void __iomem *xaga_dbg_base;
static void xaga_wdt_dbg_workfn(struct work_struct *work)
{
	if (!xaga_dbg_base)
		return;
	pr_err("XAGA-WDT: +15s MODE=%08x LENGTH=%08x\n",
	       readl(xaga_dbg_base + WDT_MODE),
	       readl(xaga_dbg_base + WDT_LENGTH));
}
static DECLARE_DELAYED_WORK(xaga_wdt_dbg_work, xaga_wdt_dbg_workfn);
...
static int mtk_wdt_probe(struct platform_device *pdev)
{
	...
	/*
	 * XAGA bring-up: expdb console records only open ~0.115 s after
	 * kernel start, so MODE readbacks printed at setup_arch time are
	 * lost. Print the raw registers here (probe lands inside the
	 * window) and again 15 s later to see whether anything in this
	 * SoC re-arms or disarms the counter behind our back.
	 */
	xaga_dbg_base = mtk_wdt->wdt_base;
	pr_err("XAGA-WDT: probe MODE=%08x LENGTH=%08x\n",
	       readl(xaga_dbg_base + WDT_MODE),
	       readl(xaga_dbg_base + WDT_LENGTH));

	schedule_delayed_work(&xaga_wdt_dbg_work, 15 * HZ);
