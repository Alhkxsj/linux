 ...
 *
 * Ported from the MT6895 vendor kernel (full feature parity):
 * timekeeping, alarm, alarm IRQ via the SPMI per-slave EINT domain,
 * power-off alarm (RTC_POFF_ALM_SET -> SPAR0/SPAR1 + PDN1/PDN2),
 * shutdown normal-power-off hook (TOP2_ELR1) and EOSC calibration.
 */
...
{
	return regmap_bulk_read(rtc->regmap, reg, val, val_count);
}
...
{
	return regmap_bulk_write(rtc->regmap, reg, val, val_count);
}

static int rtc_write(struct mt6685_rtc *rtc, unsigned int reg,
		     unsigned int val)
{
	return rtc_bulk_write(rtc, reg, &val, 2);
}
			...
			void *unused)
{
	...
	if (kpoc_alarm) {
		dev_notice(rtc->rtc_dev->dev.parent, "%s trigger reboot\n", __func__);
		complete(&rtc->comp);
		kpoc_alarm = false;
	}
	return NOTIFY_DONE;
...
static void mtk_rtc_work_queue(struct work_struct *work)
{
	...
	if (!ret) {
		...
		dev_notice(rtc->rtc_dev->dev.parent, "%s timeleft= %d\n",
			   __func__, msecs);
		rtc_mark_kpoc(rtc);
		kernel_restart("kpoc");
	}
...
static void mtk_rtc_update_pwron_alarm_flag(struct mt6685_rtc *rtc)
{
	...
	ret = rtc_update_bits(rtc, rtc->addr_base + RTC_PDN1,
			      RTC_PDN1_PWRON_TIME, 0);
	if (ret < 0)
		goto exit;

	ret = rtc_update_bits(rtc, rtc->addr_base + RTC_PDN2,
...
static int mtk_rtc_restore_alarm(struct mt6685_rtc *rtc, struct rtc_time *tm)
{
	...
	ret = rtc_write(rtc, rtc->addr_base + RTC_AL_MASK, RTC_AL_MASK_DOW);
	if (ret < 0)
		goto exit;
			    ...
			    struct rtc_time *nowtm, struct rtc_time *tm)
{
	...
	if (pdn1 & RTC_PDN1_PWRON_TIME) { /* power-on time is available */
		...
		ret = rtc_read(rtc, rtc->addr_base + RTC_SPAR0, &spar0);
		if (ret < 0)
			goto exit;

		ret = rtc_read(rtc, rtc->addr_base + RTC_SPAR1, &spar1);
		if (ret < 0)
			goto exit;
...
static int mtk_rtc_write_trigger(struct mt6685_rtc *rtc)
{
	...
	ret = rtc_write(rtc, rtc->addr_base + rtc->data->wrtgr, 1);
	if (ret < 0)
		return ret;

	while (1) {
		ret = rtc_read(rtc, rtc->addr_base + RTC_BBPU, &data);
		if (ret < 0)
			break;
...
static int mtk_rtc_is_alarm_irq(struct mt6685_rtc *rtc)
{
	...
	if ((ret == 0) && (irqsta & RTC_IRQ_STA_AL)) {
		...
		ret = rtc_write(rtc, rtc->addr_base + RTC_BBPU, bbpu);
		if (ret < 0)
			dev_err(rtc->rtc_dev->dev.parent, "%s: %d error\n",
				__func__, __LINE__);
...
static irqreturn_t mtk_rtc_irq_handler_thread(int irq, void *data)
{
	...
	if (pwron_alarm) {
		...
		now_time = mktime64(nowtm.tm_year, nowtm.tm_mon, nowtm.tm_mday,
		...
		time = mktime64(tm.tm_year, tm.tm_mon, tm.tm_mday,
				tm.tm_hour, tm.tm_min, tm.tm_sec);
...
static void mtk_rtc_set_pwron_time(struct mt6685_rtc *rtc, struct rtc_time *tm)
{
	...
	for (i = RTC_OFFSET_SEC; i < RTC_OFFSET_COUNT; i++) {
		if (i == RTC_OFFSET_DOW)
			continue;
		ret = rtc_update_bits(rtc, rtc->addr_base + rtc_pwron_reg[i][RTC_REG],
...
static void mtk_rtc_save_pwron_time(struct mt6685_rtc *rtc, bool enable, struct rtc_time *tm)
{
	u32 pdn1 = 0;
	int ret;
		...
		for (i = 0; i < RTC_OFFSET_COUNT; i++) {
			...
			if (j == retry_time)
				dev_info(rtc->rtc_dev->dev.parent,
					 "[RTC check] %s write %d, latest %d\n",
					 rtc_time_reg_name[i], data[i], latest[i]);
		}
...
/*
 * NVMEM offset mapping (gauge consumers, fg_init/fg_soc cells):
 * offset 0 -> SPARE_FG2 -> reg 0x5d4
 * offset 1 -> SPARE_FG3 -> reg 0x5d6
 * Strict single-byte cell access: bytes must be 1.
 */
static int mt6685_nvmem_read(void *priv, unsigned int offset, void *val, size_t bytes)
{
	struct mt6685_rtc *rtc = priv;
	unsigned int reg;
	...
	reg = (offset == 0) ? (rtc->addr_base + RTC_RG_FG2)
			    : (rtc->addr_base + RTC_RG_FG3);

	mutex_lock(&rtc->lock);
	ret = rtc_read(rtc, reg, &raw_val);
	mutex_unlock(&rtc->lock);

	if (ret)
		return ret;

	*(u8 *)val = (u8)(raw_val & 0xff);
	return 0;
}

static int mt6685_spare_write_one(struct mt6685_rtc *rtc, unsigned int offset, u8 val)
{
	unsigned int reg = (offset == 0) ? (rtc->addr_base + RTC_RG_FG2)
					 : (rtc->addr_base + RTC_RG_FG3);
	unsigned int orig = 0;
	int ret;
	...
	/* Magic unlock sequences for MT6685 series */
	if (offset == 0) {
		rtc_write(rtc, reg, 0xaf);
		rtc_write(rtc, reg, 0xaf);
		rtc_write(rtc, reg, 0x5e);
	} else {
		rtc_write(rtc, reg, 0x66);
		rtc_write(rtc, reg, 0x66);
		rtc_write(rtc, reg, 0xf1);
	}

	orig &= ~0xff;
	orig |= val;
	return rtc_write(rtc, reg, orig);
}

static int mt6685_nvmem_write(void *priv, unsigned int offset, void *val, size_t bytes)
{
	struct mt6685_rtc *rtc = priv;
	int ret;
	...
	mutex_lock(&rtc->lock);
	ret = mt6685_spare_write_one(rtc, offset, *(const u8 *)val);
	if (ret) {
		dev_err(rtc->rtc_dev->dev.parent, "failed to write spare reg offset %u: %d\n", offset, ret);
	} else {
		ret = mtk_rtc_write_trigger(rtc);
		if (ret)
			dev_err(rtc->rtc_dev->dev.parent, "write trigger failed: %d\n", ret);
	}
	mutex_unlock(&rtc->lock);
...
static int mtk_rtc_set_spare(struct device *dev)
{
	...
	struct nvmem_config nvmem_cfg = {
		...
		.stride = 1,
		.size = 2,
		.reg_read = mt6685_nvmem_read,
		.reg_write = mt6685_nvmem_write,
		.priv = NULL,
	...
	if (IS_ERR(nvmem)) {
		dev_err(dev, "nvmem register failed\n");
...
static int mtk_rtc_probe(struct platform_device *pdev)
{
...
#ifdef SUPPORT_PWR_OFF_ALARM
	mt6685_rtc_suspend_lock = wakeup_source_register(NULL, "mt6685-rtc suspend wakelock");

	/*
	 * Boot mode drives the KPOC (alarm reboot during power-off charging)
	 * path. "atag,boot" is provided by the vendor LK; without it stay at
	 * NORMAL_BOOT: power-on alarms still resolve (pwron_alm path) but no
	 * kernel restart loop is armed.
	 */
	...
	if (register_pm_notifier(&rtc->pm_nb))
		dev_err(&pdev->dev, "rtc pm failed\n");
	else
...
{
	struct mt6685_rtc *rtc = platform_get_drvdata(pdev);
