...
int SOC_to_OCV_c(struct mtk_battery *gm, int _soc)
{
	...
	if (profile_p == NULL) {
		return blp933_soc_to_ocv(_soc / 100);
	}
...
int DOD_to_OCV_c(struct mtk_battery *gm, int _dod)
{
	...
	profile_p = fg_get_profile(gm, ptable->temperature_tb1);
	if (profile_p == NULL) {
		return blp933_soc_to_ocv((10000 - _dod) / 100);
	}
...
int OCV_to_SOC_c(struct mtk_battery *gm, int _ocv)
{
	...
	profile_p = fg_get_profile(gm, ptable->temperature_tb1);
	if (profile_p == NULL) {
		return blp933_vbat_to_soc(_ocv / 10) * 100;
	}
...
int OCV_to_DOD_c(struct mtk_battery *gm, int _ocv)
{
	...
	profile_p = fg_get_profile(gm, ptable->temperature_tb1);
	if (profile_p == NULL) {
		return (100 - blp933_vbat_to_soc(_ocv / 10)) * 100;
	}
...
void fgr_update_c_dod(struct mtk_battery *gm)
{
	...
	int car = 0;
	int qmax = 50000; /* 5000mAh in 0.1mAh units */
	...
	fgr_update_quse(gm, 1);
	if (algo->quse_tb1 > 0)
		qmax = algo->quse_tb1 * 10;

	/* Coulomb Counter: DOD = DOD_0 - (CAR * 10000 / Qmax) */
	algo->fg_c_dod = algo->fg_c_d0_dod - (car * 10000 / qmax);
	algo->fg_c_soc = 10000 - algo->fg_c_dod;

	if (algo->fg_c_soc < 0)
		algo->fg_c_soc = 0;
	if (algo->fg_c_soc > 10000)
		algo->fg_c_soc = 10000;

	bm_debug("[%s] car:%d qmax:%d d0_dod:%d dod:%d c_soc:%d\n",
		__func__, car, qmax, algo->fg_c_d0_dod,
...
void fgr_dod_init(struct mtk_battery *gm)
{
	...
	int rtc_invalid = (gm && gm->gauge) ? gm->gauge->hw_status.rtc_invalid : 1;
	int is_bat_plugout = (gm && gm->gauge) ? gm->gauge->hw_status.is_bat_plugout : 1;
	struct mtk_battery_algo *algo = &gm->algo;
	struct fuel_gauge_table_custom_data *ptable = &gm->fg_table_cust_data;
	struct fuel_gauge_custom_data *pdata = &gm->fg_cust_data;

	algo->rtc_ui_soc = UNIT_TRANS_100 * con0_uisoc;
	init_swocv = gauge_get_int_property(GAUGE_PROP_PTIM_BATTERY_VOLTAGE) * 10;

	bm_err("[%s] pre-check: rtc_invalid=%d, is_bat_plugout=%d, con0_uisoc=%d, con0_soc=%d, init_swocv=%d\n",
		__func__, rtc_invalid, is_bat_plugout, con0_uisoc, con0_soc, init_swocv);

	/*
	 * Official 5.10 Restore Eligibility Gate:
	 * 1. FG3 Bit7 Valid == 1 (rtc_invalid == 0)
	 * 2. FG2 Bit5 Attached == 1 (is_bat_plugout == 0)
	 * 3. FG3 UI SOC != 0 and within range [1, 100] (con0_uisoc > 0 && con0_uisoc <= 100)
	 * 4. CON0 Core SOC != 0 (con0_soc != 0)
	 *
	 * Note: 0% is a valid physical SOC, but official MTK logic deliberately
	 * excludes 0% from Restore Eligibility (forcing OCV lookup) because a dead
	 * battery could have relaxed or charged while powered off, and cleared/uninitialized
	 * PMIC/NVMEM registers naturally read as 0x00.
	 */
	if (rtc_invalid == 0 && is_bat_plugout == 0 &&
	    con0_uisoc > 0 && con0_uisoc <= 100 && con0_soc != 0) {
		/*
		 * Path 2: Official Restore Path
		 */
		algo->ui_d0_soc = algo->rtc_ui_soc;
		algo->fg_c_d0_soc = UNIT_TRANS_100 * con0_soc;

		bm_err("[%s] Official Restore Path: restored ui_d0_soc=%d (con0_uisoc=%d%%), fg_c_d0_soc=%d (con0_soc=%d%%)\n",
			__func__, algo->ui_d0_soc, con0_uisoc,
			algo->fg_c_d0_soc, con0_soc);
	} else {
		/*
		 * Path 1: Official OCV Fallback Path
		 */
		...
		if (algo->rtc_ui_soc < 0) {
			bm_err("[%s] rtcui<0, set to 0, rtc_ui_soc:%d fg_c_d0_soc:%d\n",
				__func__, algo->rtc_ui_soc, algo->fg_c_d0_soc);
			algo->rtc_ui_soc = 0;
		}
		...
		bm_err("[%s] rtcui=0 case, init_swocv=%d, OCV_to_SOC_c=%d ui:[%d %d] con0_soc=[%d %d]\n",
			__func__, init_swocv, algo->fg_c_d0_soc,
			algo->ui_d0_soc, algo->rtc_ui_soc,
			con0_soc, con0_uisoc);
	}

	/*
	 * Common official post-initialization (verbatim from MTK 5.10):
	 */
