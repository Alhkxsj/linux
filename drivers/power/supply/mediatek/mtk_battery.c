...
static int __maybe_unused charger_manager_get_sic_current(void) { return 0; }
static int __maybe_unused charger_manager_set_sic_current(int cur) { return 0; }
...
/*
 * Stock keeps the authoritative cycle count in the fgdaemon and pushes it
 * back over netlink (FG_DAEMON_CMD_SET_BAT_CYCLES); there is no daemon on
 * mainline, so count in-kernel: every coulomb of discharge throughput
 * accumulates in bat_cycle_ncar and one cycle is counted per
 * bat_cycle_thr. RAM-only: the counter restarts at 0 each boot.
 */
void fg_sw_bat_cycle_accu(struct mtk_battery *gm)
{
	int fg_coulomb = 0, diff_car = 0, ret;

	if (gm->disableGM30)
		return;

	ret = gauge_get_property(GAUGE_PROP_COULOMB, &fg_coulomb);
	if (ret != 0)
		return;
	...
	/* CAR rising means charging or a gauge reset; only discharge counts */
	if (diff_car > 0)
		return;

	gm->bat_cycle_ncar += -diff_car;

	if (gm->bat_cycle_thr > 0 &&
		gm->bat_cycle_ncar >= gm->bat_cycle_thr) {
		gm->bat_cycle_ncar = 0;
		gm->bat_cycle++;
		bm_err("[fg_cycle] cycle:%d (car:%d)\n",
			gm->bat_cycle, fg_coulomb);
	}
}
...
int blp933_vbat_to_soc(int vbat_mv)
{
	static const struct {
		int mv;
		int soc;
	...
	int n = ARRAY_SIZE(ocv_tbl);
	int i;

	if (vbat_mv >= ocv_tbl[0].mv)
		return 100;
	if (vbat_mv <= ocv_tbl[n - 1].mv)
		return 0;

	for (i = 0; i < n - 1; i++) {
		if (vbat_mv >= ocv_tbl[i + 1].mv) {
			int v_high = ocv_tbl[i].mv;
			int v_low = ocv_tbl[i + 1].mv;
			int soc_high = ocv_tbl[i].soc;
			int soc_low = ocv_tbl[i + 1].soc;

			return soc_low + (vbat_mv - v_low) * (soc_high - soc_low) / (v_high - v_low);
		}
	}
	return 0;
}

int blp933_soc_to_ocv(int soc_pct)
{
	static const struct {
		int mv;
		int soc;
	...
	for (i = 0; i < n - 1; i++) {
		if (soc_pct <= ocv_tbl[i].soc && soc_pct >= ocv_tbl[i + 1].soc) {
			int v_high = ocv_tbl[i].mv * 10;
			int v_low = ocv_tbl[i + 1].mv * 10;
			int soc_high = ocv_tbl[i].soc;
			int soc_low = ocv_tbl[i + 1].soc;

			return v_low + (soc_pct - soc_low) * (v_high - v_low) / (soc_high - soc_low);
		}
	}
	return 38000;
}
	...
	union power_supply_propval *val)
{
	...
	if (gm->algo.active == true) {
		...
		ret = gauge_get_property(GAUGE_PROP_BATTERY_VOLTAGE, &live_vbat);
		if (ret == 0 && live_vbat > 2800) {
			if (live_vbat < 10000)
				bs_data->bat_capacity = blp933_vbat_to_soc(live_vbat);
			else
				bs_data->bat_capacity = blp933_vbat_to_soc(live_vbat / 1000);
		}
		if (bs_data->bat_capacity < 0)
			bs_data->bat_capacity = 0;
		if (bs_data->bat_capacity > 100)
			bs_data->bat_capacity = 100;
		gm->ui_soc = bs_data->bat_capacity;
	}
	...
	case POWER_SUPPLY_PROP_STATUS: {
		int cur = 0;
		bool is_charging_current = false;
		if (gauge_get_property(GAUGE_PROP_BATTERY_CURRENT, &cur) == 0) {
			if (cur > 100) /* > +10 mA entering battery */
				is_charging_current = true;
		}
		if (!bs_data->chg_psy) {
			bs_data->chg_psy = power_supply_get_by_name("mtk-master-charger");
		}
		if (bs_data->chg_psy) {
			union power_supply_propval online = {0}, chg_stat = {0};
			power_supply_get_property(bs_data->chg_psy, POWER_SUPPLY_PROP_ONLINE, &online);
			power_supply_get_property(bs_data->chg_psy, POWER_SUPPLY_PROP_STATUS, &chg_stat);
			if (chg_stat.intval == POWER_SUPPLY_STATUS_FULL)
				bs_data->bat_status = POWER_SUPPLY_STATUS_FULL;
			else if (chg_stat.intval == POWER_SUPPLY_STATUS_CHARGING || (online.intval && is_charging_current))
				bs_data->bat_status = POWER_SUPPLY_STATUS_CHARGING;
			else if (online.intval && chg_stat.intval == POWER_SUPPLY_STATUS_NOT_CHARGING)
				bs_data->bat_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
			else
				bs_data->bat_status = POWER_SUPPLY_STATUS_DISCHARGING;
		} else if (is_charging_current) {
			bs_data->bat_status = POWER_SUPPLY_STATUS_CHARGING;
		} else {
			bs_data->bat_status = POWER_SUPPLY_STATUS_DISCHARGING;
		}
	...
	case POWER_SUPPLY_PROP_HEALTH:
		if (gm->ti_bms_psy) {
			...
			tbat = force_get_tbat(gm, true) * 10;
			val->intval = tbat;
		}
		if (tbat <= -100)
	...
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		if (gm->ti_bms_psy)
			ret = power_supply_get_property(gm->ti_bms_psy, POWER_SUPPLY_PROP_CYCLE_COUNT, val);
		else
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (gm->ti_bms_psy) {
			ret = power_supply_get_property(gm->ti_bms_psy, POWER_SUPPLY_PROP_CURRENT_NOW, val);
		} else {
			int cur = 0;
			ret = gauge_get_property(GAUGE_PROP_BATTERY_CURRENT, &cur);
			if (ret == 0) {
				val->intval = cur * 100;
	...
	case POWER_SUPPLY_PROP_POWER_NOW: {
		int cur = 0, vbat = 0;
		int ret_c = gauge_get_property(GAUGE_PROP_BATTERY_CURRENT, &cur);
		int ret_v = gauge_get_property(GAUGE_PROP_BATTERY_VOLTAGE, &vbat);
		if (ret_c < 0 || ret_v < 0 || vbat <= 0) {
			ret = -ENODATA;
			break;
		}
		if (vbat < 10000)
			vbat *= 1000;
		val->intval = (int)(((u64)abs(cur * 100) * (u64)vbat) / 1000000ULL);
		ret = 0;
		break;
	}
	...
	case POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN:
		/*
		 * BLP933: 5 Ah typical at 3.87 V nominal = 19.35 Wh
		 * (OnePlus spec: 5000 mAh/19.35 Wh typical, 4890 mAh rated).
		 * Reporting this ourselves stops UPower from deriving
		 * charge_full_design * voltage_max_design = 5 Ah * 4.45 V
		 * (charge cutoff, not nominal) = 22.25 Wh.
		 */
		val->intval = 19350000;
		break;
	case POWER_SUPPLY_PROP_ENERGY_FULL:
		/*
		 * Same as design capacity, mirroring stock's standard
		 * interface (CHARGE_FULL = static q_max). UPower 1.91
		 * switches to CHARGE units and derives design energy as
		 * charge_full_design * voltage_max_design (5 Ah * 4.45 V
		 * = 22.25 Wh) unless energy_full is a valid number.
		 */
		val->intval = 19350000;
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		if (gm->ti_bms_psy) {
			ret = power_supply_get_property(gm->ti_bms_psy, POWER_SUPPLY_PROP_CHARGE_COUNTER, val);
		} else {
			int fg_coulomb = 0;
			ret = gauge_get_property(GAUGE_PROP_COULOMB, &fg_coulomb);
			if (ret == 0)
				val->intval = fg_coulomb * 100;
		}
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		...
		if (gm->ti_bms_psy) {
			ret = power_supply_get_property(gm->ti_bms_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, val);
		} else {
			int vbat = 0;
			ret = gauge_get_property(GAUGE_PROP_BATTERY_VOLTAGE, &vbat);
			if (ret == 0 && vbat > 0) {
				if (vbat < 10000)
					val->intval = vbat * 1000;
				else
					val->intval = vbat;
			}
		}
	...
	case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
		/* full, unknown, or discharging must return 0 */
		...
		if ((ret == POWER_SUPPLY_CAPACITY_LEVEL_FULL) ||
			...
			int current_now = 0;
			ret = gauge_get_property(GAUGE_PROP_BATTERY_CURRENT, &current_now);
			if (ret == -EHOSTDOWN)
				current_now = gm->ibat;
			/* current_now in 0.1mA; need positive charging current > 5mA */
			if (ret == 0 && current_now > 50) {
				int remain_ui = 100 - bs_data->bat_capacity;
				u64 remain_0_1mah = (u64)remain_ui * 500ULL;
				val->intval = (int)((remain_0_1mah * 3600ULL) / (u64)current_now);
			} else {
				val->intval = 0;
			}
			ret = 0;
	...
	const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		if (gm->bs_data.chg_psy) {
			union power_supply_propval chg_val = { .intval = val->intval };
			rc = power_supply_set_property(gm->bs_data.chg_psy,
				POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT, &chg_val);
		}
		break;
	default:
		rc = -EINVAL;
...
void battery_service_data_init(struct mtk_battery *gm)
{
	...
	/* one cycle = design qmax of discharge throughput (0.1 mAh units);
	 * stock's fgdaemon set this over netlink, none exists on mainline */
	gm->bat_cycle_thr = 50000;
}
...
int adc_battemp(struct mtk_battery *gm, int res)
{
	int i = 0;
	int res1 = 0, res2 = 0;
	int tbatt_value = 25, tmp1 = 0, tmp2 = 0;
	...
	if (res >= ptable[0].TemperatureR)
		return ptable[0].BatteryTemp;
	if (res <= ptable[max_idx].TemperatureR)
		return ptable[max_idx].BatteryTemp;
...
int volttotemp(struct mtk_battery *gm, int dwVolt, int volt_cali)
{
	...
	ret = gauge_get_property(GAUGE_PROP_BIF_VOLTAGE, &vbif28_raw);
	if (ret == 0 && vbif28_raw > 1000 && vbif28_raw < 3500)
		vbif28 = vbif28_raw;

	/* If dwVolt is in uV (> 10000), convert to mV */
	if (dwVolt > 10000)
		dwVolt = dwVolt / 1000;
	if (vbif28 > 10000)
		vbif28 = vbif28 / 1000;
	...
	/* R_NTC = R_pullup (100k) * V_adc / (V_ref - V_adc) */
	tres_temp = (100000LL * (long long)dwVolt);
	tres = div_s64(tres_temp, delta_v);

	sbattmp = adc_battemp(gm, (int)tres);

	bm_debug("[volttotemp] dwVolt=%d mV, vbif28=%d mV, delta_v=%d mV, tres=%lld ohm, sbattmp=%d C\n",
		dwVolt, vbif28, delta_v, tres, sbattmp);
...
int force_get_tbat_internal(struct mtk_battery *gm, bool update)
{
	...
	int bat_temperature_val = 25;
	int ret = 0;

	ret = gauge_get_property(GAUGE_PROP_BATTERY_TEMPERATURE_ADC,
		&bat_temperature_volt);
	bm_debug("[force_get_tbat_internal] ret=%d, bat_temperature_volt=%d\n", ret, bat_temperature_volt);
	...
	if (bat_temperature_volt > 0) {
		bat_temperature_val = volttotemp(gm, bat_temperature_volt, 0);
	}
	...
	int val)
{
	...
	if (attr[gp].prop == gp) {
		mutex_lock(&gauge->ops_lock);
		ret = attr[gp].set(gauge, &attr[gp], val);
		mutex_unlock(&gauge->ops_lock);
	} else {
		bm_err("%s gp:%d idx error\n", __func__, gp);
		return -ENOTSUPP;
...
/*
 * RAM-only cycle counter (fg_sw_bat_cycle_accu); zeroed at every boot.
 * Userspace (qqcandy-battery-tracker) re-seeds the persisted lifetime
 * value here at boot and flushes the max back to its state file, the
 * same daemon-fed shape stock's fgdaemon had via netlink SET_BAT_CYCLES.
 */
static int bat_cycle_get(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int *val)
{
	*val = gm->bat_cycle;
...
static int bat_cycle_set(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int val)
{
	if (val > gm->bat_cycle)
...
static struct mtk_battery_sysfs_field_info battery_sysfs_field_tbl[] = {
	...
	BAT_SYSFS_FIELD_RW(bat_cycle, BAT_PROP_BAT_CYCLE),
};
...
static int mtk_battery_check_and_commit_stable_cycle(struct mtk_battery *gm)
{
	int ret, live_vbat = 0, live_vbat_mv = 0;
	int ui_soc_pct, core_soc_pct;
	int read_core_soc = 0, read_ui_soc = 0;
	u8 fg2_val, fg3_val;
	int rtc_invalid, is_bat_plugout;

	if (!gm || !gm->gauge)
		return -ENODEV;

	/* One-shot guard: already validated */
	if (gm->rtc_validated)
		return 0;

	/* 1. Stable Gate Qualification */
	if (battery_get_int_property(BAT_PROP_INIT_DONE) != 1)
		return 0;

	if (gauge_get_int_property(GAUGE_PROP_BATTERY_EXIST) != 1)
		return 0;

	/* Candidate D Plausibility Gate: 2800mV ~ 4600mV */
	ret = gauge_get_property(GAUGE_PROP_BATTERY_VOLTAGE, &live_vbat);
	if (ret) {
		bm_err("[%s] Stable Gate: vbat read failed: %d\n", __func__, ret);
		return ret;
	}
	live_vbat_mv = (live_vbat < 10000) ? live_vbat : (live_vbat / 1000);
	if (live_vbat_mv < 2800 || live_vbat_mv > 4600) {
		bm_err("[%s] Stable Gate skipped: vbat=%dmV outside [2800, 4600]mV\n",
			__func__, live_vbat_mv);
		return 0;
	}

	/* Ensure algorithm capacity table initialized */
	if (gm->algo.quse_tb1 <= 0)
		return 0;

	/* Update coulomb-integrated DOD and state */
	fgr_update_c_dod(gm);

	/* Check algorithm SOC bounds: [1%, 100%] -> [100, 10000] in basis points */
	if (gm->algo.ui_soc < 100 || gm->algo.ui_soc > 10000)
		return 0;
	if (gm->algo.soc < 100 || gm->algo.soc > 10000)
		return 0;

	ui_soc_pct = (gm->algo.ui_soc + 50) / 100;
	core_soc_pct = (gm->algo.soc + 50) / 100;
	if (ui_soc_pct < 1 || ui_soc_pct > 100 || core_soc_pct < 1 || core_soc_pct > 100)
		return 0;

	bm_err("[%s] Stable Cycle Gate PASSED (vbat=%dmV ui_soc=%d (%d%%) core_soc=%d (%d%%)). Beginning 4-step commit...\n",
		__func__, live_vbat_mv, gm->algo.ui_soc, ui_soc_pct, gm->algo.soc, core_soc_pct);

	/* 2. Step 1: Write CON0 Core SOC */
	ret = gauge_set_property(GAUGE_PROP_CON0_SOC, core_soc_pct * 100);
	if (ret) {
		bm_err("[%s] Step 2 FAIL: GAUGE_PROP_CON0_SOC error %d\n", __func__, ret);
		return ret;
	}

	/* 3. Step 2: Write FG3 UI SOC payload (Bit7 remains 0) */
	ret = gauge_set_property(GAUGE_PROP_RTC_UI_SOC, ui_soc_pct);
	if (ret) {
		bm_err("[%s] Step 3 FAIL: GAUGE_PROP_RTC_UI_SOC error %d\n", __func__, ret);
		return ret;
	}

	/* 4. Step 3 & 4: Clear FG2 unplug and COMMIT FG3 Bit7 Valid */
	ret = gauge_set_property(GAUGE_PROP_RESET_FG_RTC, 0);
	if (ret) {
		bm_err("[%s] Step 4 FAIL: GAUGE_PROP_RESET_FG_RTC error %d\n", __func__, ret);
		return ret;
	}

	/* 5, 6, 7. Read-back verification */
	ret = gauge_get_property(GAUGE_PROP_CON0_SOC, &read_core_soc);
	if (ret) {
		bm_err("[%s] Readback FAIL: CON0 read error %d\n", __func__, ret);
		return ret;
	}
	ret = gauge_get_property(GAUGE_PROP_RTC_UI_SOC, &read_ui_soc);
	if (ret) {
		bm_err("[%s] Readback FAIL: RTC_UI_SOC read error %d\n", __func__, ret);
		return ret;
	}
	fg2_val = gm->gauge->hw_status.gspare0_reg;
	fg3_val = gm->gauge->hw_status.gspare3_reg;
	rtc_invalid = gm->gauge->hw_status.rtc_invalid;
	is_bat_plugout = gm->gauge->hw_status.is_bat_plugout;

	/* Step 5: FG2 read-back */
	if (is_bat_plugout != 0 || (fg2_val & 0x20) == 0) {
		bm_err("[%s] Step 5 FAIL: FG2 read-back mismatch: plugout=%d fg2=0x%02x\n",
			__func__, is_bat_plugout, fg2_val);
		return -EIO;
	}

	/* Step 6: FG3 read-back */
	if (rtc_invalid != 0 || (fg3_val & 0x80) == 0 || read_ui_soc != ui_soc_pct) {
		bm_err("[%s] Step 6 FAIL: FG3 read-back mismatch: rtc_invalid=%d fg3=0x%02x read_ui=%d expected=%d\n",
			__func__, rtc_invalid, fg3_val, read_ui_soc, ui_soc_pct);
		return -EIO;
	}

	/* Step 7: CON0 read-back */
	if (read_core_soc != core_soc_pct) {
		bm_err("[%s] Step 7 FAIL: CON0 read-back mismatch: read_core=%d expected=%d\n",
			__func__, read_core_soc, core_soc_pct);
		return -EIO;
	}

	/* ALL CHECKS VERIFIED IN HARDWARE: Commit state */
	gm->rtc_validated = true;
	bm_err("[%s] P2-B2 Lifecycle Commit & Read-back SUCCESS! rtc_validated=true (UI:%d%% Core:%d%% FG2:0x%02x FG3:0x%02x CON0:%d)\n",
		__func__, ui_soc_pct, core_soc_pct, fg2_val, fg3_val, read_core_soc);
...
int battery_update_routine(void *arg)
{
	...
	while (1) {
		...
		if (!gm->rtc_validated)
			mtk_battery_check_and_commit_stable_cycle(gm);
in_sleep:
...
int battery_init(struct platform_device *pdev)
{
	...
	/* Mainline Linux: always run in kernel-mode battery algorithm */
	gm->algo.active = true;
