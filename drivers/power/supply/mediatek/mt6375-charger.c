...
static void mt6375_chg_bc12_work_func(struct work_struct *work)
{
	...
	{
		u32 live_vbus = 0;
		ret = mt6375_get_vbus(ddata->chgdev, &live_vbus);
		if (ret == 0 && (live_vbus >= 4400000 || (live_vbus >= 4400 && live_vbus < 20000)))
			attach = true;
	}

	mt_dbg(ddata->dev, "attach=%d\n", attach);
	if (attach) {
		/* Always enable charging immediately on attach so PD power banks and Type-C sources charge right away */
		mt6375_chg_enable_charging(ddata, true);
		mt6375_chg_field_set(ddata, F_VBUS_OV, 14500);
		mt6375_chg_field_set(ddata, F_IAICR, 3000);
		mt6375_chg_field_set(ddata, F_CC, 3150);
		mt6375_chg_field_set(ddata, F_VMIVR, 4400);
		mt6375_chg_field_set(ddata, F_HZ, 0);
		mt6375_chg_field_set(ddata, F_WDT_EN, 0);
		mt6375_chg_field_set(ddata, F_ILIM_EN, 0);
...
static int mt6375_plug_in(struct charger_device *chgdev)
{
	...
	mt_dbg(ddata->dev, "++\n");
	mt6375_chg_field_set(ddata, F_WDT_EN, 0);
	mt6375_chg_field_set(ddata, F_WDT_RST, 1);
	return 0;
}
