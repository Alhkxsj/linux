...
static u8 get_rtc_spare0_fg_value(struct mtk_gauge *gauge)
{
	...
	nvmem_cell_put(cell);
	if (IS_ERR(buf)) {
		bm_err("[%s] read rtc cell fail: %ld\n", __func__, PTR_ERR(buf));
		return 0;
	}
...
static int set_rtc_spare0_fg_value(struct mtk_gauge *gauge, u8 val)
{
	struct nvmem_cell *cell;
	u32 length = 1;
	int ret;
	...
	nvmem_cell_put(cell);
	if (ret != length) {
		bm_err("[%s] write rtc cell fail: %d\n", __func__, ret);
...
static int set_rtc_spare_fg_value(struct mtk_gauge *gauge, u8 val)
{
	struct nvmem_cell *cell;
	u32 length = 1;
	int ret;
			    ...
			    int val)
{
	...
	/* read current spare0 and spare3 */
	spare0_reg = get_rtc_spare0_fg_value(gauge);
	spare3_reg = get_rtc_spare_fg_value(gauge);

	/*
	 * Phase 1: Clear battery unplug status in FG2 (SPARE_FG2, 0x5d4)
	 * Pulse Bit7 (0x80) -> 1ms -> Bit5 (0x20)
	 */
	if ((hw_id & 0xff00) == 0x3500) {
		temp_value = 0x80;
		ret = set_rtc_spare0_fg_value(gauge, temp_value);
		if (ret)
			return ret;
		mdelay(1);
		temp_value = 0x00;
	...
	/*
	 * Phase 2: Read back FG2 immediately and verify unplug clear.
	 * If clearing unplug failed, WE MUST NOT COMMIT VALID BIT TO FG3!
	 */
	after_rst_spare0_reg = get_rtc_spare0_fg_value(gauge);
	if ((after_rst_spare0_reg & 0x20) == 0) {
		bm_err("[%s] FG2 Bit5 clear unplug verify failed: 0x%02x\n",
			__func__, after_rst_spare0_reg);
		return -EIO;
	}

	/*
	 * Phase 3: FINAL COMMIT POINT - Set Bit7 of FG3 (SPARE_FG3, 0x5d6)
	 * Read current spare3 (preserving bits[6:0] UI SOC written by Step 2)
	 * and set Bit7 = 1.
	 */
	spare3_reg = get_rtc_spare_fg_value(gauge);
	ret = set_rtc_spare_fg_value(gauge, (u8)((spare3_reg & 0x7f) | 0x80));
	if (ret)
		return ret;

	/*
	 * Phase 4: Read back FG3 immediately and verify Valid bit.
	 */
	after_rst_spare3_reg = get_rtc_spare_fg_value(gauge);
	if ((after_rst_spare3_reg & 0x80) == 0) {
		bm_err("[%s] FG3 Bit7 valid bit verify failed: 0x%02x\n",
			__func__, after_rst_spare3_reg);
		return -EIO;
	}

	/*
	 * Phase 5: Re-read hardware status and synchronize in-memory state.
	 */
	fgauge_read_RTC_boot_status(gauge);

	if (gauge->hw_status.rtc_invalid != 0 || gauge->hw_status.is_bat_plugout != 0) {
		bm_err("[%s] hw_status sync failed: rtc_invalid=%d is_bat_plugout=%d\n",
			__func__, gauge->hw_status.rtc_invalid, gauge->hw_status.is_bat_plugout);
		return -EIO;
	}

	bm_err("[fgauge_reset_fg_rtc] verified SUCCESS: spare0 0x%02x -> 0x%02x, spare3 0x%02x -> 0x%02x\n",
			   ...
			   int *val)
{
	...
	if (IS_ERR_OR_NULL(gauge->chan_bif)) {
		bm_err("[%s]chan error\n", __func__);
		return -ENOTSUPP;
	}

	ret = iio_read_channel_processed(gauge->chan_bif, val);
	bm_debug("[bif_voltage_get] chan_bif=%p, ret=%d, val=%d\n", gauge->chan_bif, ret, *val);
	return ret;
}
				       ...
				       struct mtk_gauge_sysfs_field_info *attr, int *val)
{
	...
	if (IS_ERR_OR_NULL(gauge->chan_bat_temp)) {
		bm_err("[%s]chan error\n", __func__);
		return -ENOTSUPP;
	}

	ret = iio_read_channel_processed(gauge->chan_bat_temp, val);
	bm_debug("[battery_temperature_adc_get] chan_bat_temp=%p, ret=%d, val=%d\n", gauge->chan_bat_temp, ret, *val);
	if (ret < 0)
		bm_err("[%s]read fail,ret=%d\n", __func__, ret);
...
static int bat_vol_get(struct mtk_gauge *gauge, struct mtk_gauge_sysfs_field_info *attr, int *val)
{
	...
	if (IS_ERR_OR_NULL(gauge->chan_bat_voltage)) {
		bm_err("[%s]chan error\n", __func__);
		return -ENOTSUPP;
	}
...
static int mt6375_gauge_probe(struct platform_device *pdev)
{
...

	{
		...
		cell = nvmem_cell_get(&pdev->dev, "initialization");
		if (IS_ERR(cell)) {
		...
		cell = nvmem_cell_get(&pdev->dev, "state-of-charge");
		if (IS_ERR(cell)) {
			if (PTR_ERR(cell) == -EPROBE_DEFER) {
				ret = -EPROBE_DEFER;
				goto out_irq_chip;
