...
/* fb node the boot logo was last painted on; -1 = not yet drawn */
static int fbcon_logo_fb_node = -1;
/* console mappings */
			       ...
			       int cols, int rows, int new_cols, int new_rows)
{
	...
	if (info->fbops->owner) {
		pr_err("XAGA-LOGO: skip - fb driver is a module\n");
		logo_shown = FBCON_LOGO_DONTSHOW;
		return;
	}

	logo_height = fb_prepare_logo(info, ops->rotate);
	pr_err("XAGA-LOGO: prepare h=%d shown=%d node=%d\n",
	       logo_height, logo_shown, info->node);
	if (logo_height <= 0)
		return;
...
static void fbcon_init(struct vc_data *vc, bool init)
{
	...
	/*
	 * XAGA: upstream hides the boot logo when the console starts
	 * quiet (loglevel <= CONSOLE_LOGLEVEL_QUIET). We always want the
	 * CPU-count Tux row during bring-up regardless of loglevel.
	 */
	if (logo_shown < 0)
		logo_shown = FBCON_LOGO_CANSHOW;
...
static bool fbcon_switch(struct vc_data *vc)
{
	...
	if (logo_shown >= 0 && fbcon_logo_fb_node != info->node &&
	    logo_lines > 0) {
		/*
		 * Console moved to a different fb (e.g. simplefb -> DRM
		 * fbdev takeover). The one-shot logo state machine would
		 * never paint again; re-arm it so the CPU-count Tux row
		 * also appears on the new framebuffer.
		 */
		logo_shown = FBCON_LOGO_DRAW;
		vc->vc_top = logo_lines;
	}
	...
	if (logo_shown == FBCON_LOGO_DRAW) {
		...
		fbcon_logo_fb_node = info->node;
		pr_err("XAGA-LOGO: drawing Tux row on fb node %d\n",
		       info->node);
