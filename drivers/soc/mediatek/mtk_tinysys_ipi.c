...
/*
 * Bring-up diagnostics: how long does a POLLING send keep interrupts off?
 *
 * IPI_SEND_POLLING takes pin_lock with spin_lock_irqsave() and then busy-waits
 * in ipi_delay() until the coprocessor acks, for up to the caller's timeout --
 * 10 s for every gpueb client on MT6895. Six captured hard resets each had
 * exactly one core stuck in __delay() with daif=IF, one of them naming
 * mtk_ipi_send_compl outright, so the real length of this window is the open
 * question. Healthy MT6895 gpueb commits measure 1.2-1.3 ms at p50 and 1.7 ms
 * worst case, hence the 5 ms default: silent in normal operation, loud if the
 * window ever grows.
 *
 * KERN_CRIT rather than KERN_WARNING on purpose. The sample that matters is
 * the last one before a hard reset, and it only survives there if it beats the
 * runtime console loglevel into the pstore console ring.
 */
static unsigned int ipi_poll_warn_us = 5000;
module_param(ipi_poll_warn_us, uint, 0644);
MODULE_PARM_DESC(ipi_poll_warn_us,
	"Report POLLING sends holding IRQs off longer than this, in us (0 disables)");

static unsigned int ipi_poll_max_us;
module_param(ipi_poll_max_us, uint, 0444);
MODULE_PARM_DESC(ipi_poll_max_us, "Longest POLLING send observed, in us");


static void ipi_isr_cb(struct mtk_mbox_pin_recv *pin, void *priv);

static void ipi_poll_report(struct mtk_ipi_device *ipidev, int ipi_id,
		u64 busy_ns, u64 send_ns, int ret)
{
	unsigned int busy_us = div_u64(busy_ns, 1000);

	if (busy_us > ipi_poll_max_us)
		ipi_poll_max_us = busy_us;

	if (!ipi_poll_warn_us || busy_us < ipi_poll_warn_us)
		return;

	pr_crit("%s IPI %d POLLING kept IRQs off %u us (send %llu, poll %llu, trysend %d, polling %d, ret %d)\n",
		ipidev->name, ipi_id, busy_us, div_u64(send_ns, 1000),
		div_u64(busy_ns - send_ns, 1000),
		ipidev->table[ipi_id].trysend_count,
		ipidev->table[ipi_id].polling_count, ret);
}
		...
		int opt, void *data, int len, unsigned long timeout)
{
	...
	unsigned long flags = 0;
	u64 ts_start = 0, ts_sent = 0, ts_done;
	u64 timeover;
	int ret;
	...
	if (ret) {
		ts_done = ts_start ? cpu_clock(0) : 0;
		...
		if (ts_start)
			ipi_poll_report(ipidev, ipi_id, ts_done - ts_start,
					ts_done - ts_start, ret);
