...
static unsigned long timespec64_to_ms(struct timespec64 *begin,
...
{
	const char *drv_name[] = { "bt ", "fm ", "gps ", "wifi ", "conninfra" };
	char buf[256] = { '\0' };
	unsigned int i, buf_len = 0, str_len;

	for (i = 0; i < CONNDRV_TYPE_MAX; i++) {
		str_len = strlen(drv_name[i]);
		if (g_conninfra_ctx.drv_inst[i].drv_status &&
		    (buf_len + str_len < 256)) {
...
static int opfunc_power_on_internal(unsigned int drv_type)
{
	...
	if (g_conninfra_ctx.infra_drv_status == DRV_STS_POWER_OFF) {
		...
		if (ret) {
			pr_err("[Conninfra Pwr On] consys_hw_enable_power_dump fail, ret=%d",
			       ret);
	...
	if (drv_type == CONNDRV_TYPE_CONNINFRA) {
		...
		for (i = 0; i < CONNDRV_TYPE_MAX; i++) {
			if (g_conninfra_ctx.drv_inst[i].drv_status ==
				...
				g_conninfra_ctx.drv_inst[i].drv_status =
					DRV_STS_POWER_OFF;
			}
		}
...
static int opfunc_chip_rst(struct msg_op_data *op)
{
	...
	while (atomic_read(&g_conninfra_ctx.rst_state) != subdrv_all_done) {
		ret = down_timeout(&g_conninfra_ctx.rst_sema,
				   msecs_to_jiffies(CONNINFRA_RESET_TIMEOUT));
		...
		for (i = 0; i < CONNDRV_TYPE_MAX; i++) {
			if ((cur_rst_state & (0x1 << i)) == 0) {
				pr_info("[chip_rst] [%s] pre-callback is not back",
					drv_thread_name[i]);
				drv_inst = &g_conninfra_ctx.drv_inst[i];
				osal_thread_show_stack(
					&drv_inst->msg_ctx.thread);
			}
		}
	}
...
#define CAL_DRV_COUNT 2
	...
	int i, ret, cur_state;
	int bt_cal_ret = 0, wf_cal_ret = 0;
	struct subsys_drv_inst *drv_inst;
	struct subsys_drv_inst *bt_drv =
		&g_conninfra_ctx.drv_inst[CONNDRV_TYPE_BT];
	struct subsys_drv_inst *wifi_drv =
		&g_conninfra_ctx.drv_inst[CONNDRV_TYPE_WIFI];
	bool bt_pre_cal = bt_drv->ops_cb.pre_cal_cb.pwr_on_cb != NULL &&
			  bt_drv->ops_cb.pre_cal_cb.do_cal_cb != NULL;
	bool wifi_pre_cal = wifi_drv->ops_cb.pre_cal_cb.pwr_on_cb != NULL &&
			    wifi_drv->ops_cb.pre_cal_cb.do_cal_cb != NULL;
	int pre_cal_done_state = 1 << CONNDRV_TYPE_WIFI;
	struct timespec64 begin, bt_cal_begin, wf_cal_begin, end, backup_end;
	unsigned int cal_result_offset = 0, cal_result_size = 0;
	int get_cal_ret;

	if (!wifi_pre_cal) {
		pr_err("[pre_cal] WiFi callbacks are not registered");
		return -ENODEV;
	}
	if (bt_pre_cal)
		pre_cal_done_state |= 1 << CONNDRV_TYPE_BT;
	else
		pr_info("[pre_cal] BT callbacks absent; run WiFi-only calibration");

	/* Check the participating subsystem status again. */
	...
	for (i = 0; i < CAL_DRV_COUNT; i++) {
		if (cal_drvs[i] == CONNDRV_TYPE_BT && !bt_pre_cal)
			continue;
		if (g_conninfra_ctx.drv_inst[cal_drvs[i]].drv_status ==
		    DRV_STS_POWER_ON) {
	...
	/* Ask only registered subdrivers to perform their pre-cal power-on. */
	...
	while ((atomic_read(&g_conninfra_ctx.pre_cal_state) &
		pre_cal_done_state) != pre_cal_done_state) {
		ret = down_timeout(&g_conninfra_ctx.pre_cal_sema,
				   msecs_to_jiffies(CONNINFRA_PRE_CAL_TIMEOUT));
		if (ret == 0)
			continue;
		cur_state = atomic_read(&g_conninfra_ctx.pre_cal_state);
		pr_info("[pre_cal] cur state=[%d] expected=[%d]", cur_state,
			pre_cal_done_state);
		if (bt_pre_cal && (cur_state & (1 << CONNDRV_TYPE_BT)) == 0) {
			pr_info("[pre_cal] BT pwr_on callback is not back");
			osal_thread_show_stack(&bt_drv->msg_ctx.thread);
		}
		if ((cur_state & (1 << CONNDRV_TYPE_WIFI)) == 0) {
			pr_info("[pre_cal] WIFI pwr_on callback is not back");
			osal_thread_show_stack(&wifi_drv->msg_ctx.thread);
		}
	}
	...
	if (bt_pre_cal) {
		bt_cal_ret = msg_thread_send_wait_1(
			&bt_drv->msg_ctx, INFRA_SUBDRV_OPID_CAL_DO_CAL, 0,
			CONNDRV_TYPE_BT);
		pr_info("[pre_cal] driver [BT] calibration %s, ret=[%d]\n",
			(bt_cal_ret == CONNINFRA_CB_RET_CAL_FAIL_POWER_OFF ||
			 bt_cal_ret == CONNINFRA_CB_RET_CAL_FAIL_POWER_ON) ?
				"fail" :
				"success",
			bt_cal_ret);
		conninfra_core_power_off(CONNDRV_TYPE_BT);
	...
	wf_cal_ret = msg_thread_send_wait_1(&wifi_drv->msg_ctx,
					    INFRA_SUBDRV_OPID_CAL_DO_CAL, 0,
					    CONNDRV_TYPE_WIFI);
	pr_info("[pre_cal] driver [WIFI] calibration %s, ret=[%d]\n",
		(wf_cal_ret == CONNINFRA_CB_RET_CAL_FAIL_POWER_OFF ||
		 wf_cal_ret == CONNINFRA_CB_RET_CAL_FAIL_POWER_ON) ?
			"fail" :
			"success",
		wf_cal_ret);
	...
	if (wifi_drv->ops_cb.pre_cal_cb.get_cal_result_cb != NULL) {
		get_cal_ret = msg_thread_send_wait_3(
			&wifi_drv->msg_ctx, INFRA_SUBDRV_OPID_GET_CAL_RESULT, 0,
			CONNDRV_TYPE_WIFI, (size_t)&cal_result_offset,
			(size_t)&cal_result_size);
		if (get_cal_ret == 0 && cal_result_size != 0) {
			ret = conninfra_core_pre_cal_backup(cal_result_offset,
							    cal_result_size);
			if (ret)
				pr_err("[pre_cal] backup error: %d", ret);
		} else {
			pr_info("[pre_cal] get_cal_ret=%d, cal_result_size=%d, cal_result_offset=0x%08x",
				get_cal_ret, cal_result_size,
				cal_result_offset);
		}
	} else {
	...
	pr_info("[pre_cal] summary mode=%s pwr=[%lu] bt_cal=[%d][%lu] wf_cal=[%d][%lu] backup=[%lu]",
		bt_pre_cal ? "BT+WiFi" : "WiFi-only",
		timespec64_to_ms(&begin, &bt_cal_begin), bt_cal_ret,
		timespec64_to_ms(&bt_cal_begin, &wf_cal_begin), wf_cal_ret,
		timespec64_to_ms(&wf_cal_begin, &end),
...
static int opfunc_subdrv_get_cal_result(struct msg_op_data *op)
{
	...
	if (drv_inst->ops_cb.pre_cal_cb.get_cal_result_cb) {
		ret = drv_inst->ops_cb.pre_cal_cb.get_cal_result_cb(offset,
								    size);
		if (ret)
...
static void conninfra_detect_time_change(void)
{
	...
	if (counter != 0) {
		...
		soc_time_diff_ms =
			(unsigned long)(curr_soc_time - prev_soc_time);
		soc_utc_diff_ms = max(soc_time_diff_ms, utc_diff) -
				  min(soc_time_diff_ms, utc_diff);
	...
	if (time_changed) {
		for (i = 0; i < CONNDRV_TYPE_MAX; i++) {
			drv_inst = &g_conninfra_ctx.drv_inst[i];
			ret = msg_thread_send_1(&drv_inst->msg_ctx,
						INFRA_SUBDRV_OPID_TIME_CHANGED,
						i);
		}
	}
}
...
static int opfunc_rfspi_write(struct msg_op_data *op)
{
	...
	/* DO spi write */
	ret = consys_hw_spi_write(op->op_data[0], op->op_data[1],
				  op->op_data[2]);
err:
	osal_unlock_sleepable_lock(&g_conninfra_ctx.core_lock);
	return ret;
...
static int opfunc_rfspi_update_bits(struct msg_op_data *op)
{
	...
	ret = consys_hw_spi_update_bits(op->op_data[0], op->op_data[1],
					op->op_data[2], op->op_data[3]);
err:
	osal_unlock_sleepable_lock(&g_conninfra_ctx.core_lock);
	return ret;
}
...
static int opfunc_pre_cal_prepare(struct msg_op_data *op)
{
	...
	if (wifi_drv->ops_cb.pre_cal_cb.do_cal_cb == NULL) {
		pr_info("[%s] [pre_cal] WiFi callback is missing, bt=[%p] wf=[%p]",
			__func__, bt_drv->ops_cb.pre_cal_cb.do_cal_cb,
	...
	if (bt_drv->ops_cb.pre_cal_cb.do_cal_cb == NULL)
		pr_info("[%s] [pre_cal] BT callback absent, use WiFi-only mode",
			__func__);
	spin_unlock_irqrestore(&g_conninfra_ctx.infra_lock, flag);
	...
	while (!ret) {
		...
		if ((cur_status == PRE_CAL_NOT_INIT ||
		     cur_status == PRE_CAL_NEED_RESCHEDULE) &&
			...
			pr_info("[%s] [pre_cal] bt=[%d] wf=[%d] status=[%d]",
				__func__, bt_drv->drv_status,
				wifi_drv->drv_status, cur_status);
		}
...
static int opfunc_dump_power_state(struct msg_op_data *op)
{
	...
	ret = consys_hw_dump_power_state((char *)op->op_data[0],
					 op->op_data[1]);
	if (ret)
...
static int opfunc_subdrv_pre_reset(struct msg_op_data *op)
{
	...
	if (/*drv_inst->drv_status == DRV_ST_POWER_ON &&*/
		...
		ret = drv_inst->ops_cb.rst_cb.pre_whole_chip_rst(
			g_conninfra_ctx.trg_drv, g_conninfra_ctx.trg_reason);
		if (ret)
	...
	pr_info("[%s] rst_state=[%d]", drv_thread_name[drv_type],
		cur_rst_state);
...
int conninfra_core_power_on(enum consys_drv_type type)
{
	...
	ret = msg_thread_send_wait_1(&infra_ctx->msg_ctx, CONNINFRA_OPID_PWR_ON,
				     0, type);
	if (ret) {
		pr_err("[%s] fail, ret = %d\n", __func__, ret);
		return ret;
...
int conninfra_core_pre_cal_start(void)
{
	...
	case PRE_CAL_ALL_DISABLED:
		pr_info("[%s] [pre_cal] Skip all pre-cal, caller = %u",
			__func__, caller);
		skip = true;
	...
	if (skip) {
		pr_info("[%s] [pre_cal] Reset status to %d", __func__,
			cal_info->status);
		osal_unlock_sleepable_lock(&cal_info->pre_cal_lock);
		return -2;
	}
...
int conninfra_core_pre_cal_backup(unsigned int offset, unsigned int size)
{
	...
	ret = msg_thread_send_wait_2(&infra_ctx->msg_ctx,
				     CONNINFRA_OPID_PRE_CAL_BACKUP, 0, offset,
				     size);
	if (ret)
		pr_err("[%s] fail, ret = %d\n", __func__, ret);
	return ret;
...
int conninfra_core_reg_readable(void)
{
	...
	if (rst_status >= CHIP_RST_RESET && rst_status < CHIP_RST_POST_CB)
		return 0;
...
int conninfra_core_reg_read(unsigned long address, unsigned int *value,
			    unsigned int mask)
{
	int ret = 0;
	struct conninfra_ctx *infra_ctx = &g_conninfra_ctx;
...
int conninfra_core_reg_write(unsigned long address, unsigned int value,
			     unsigned int mask)
{
	int ret = 0;
	struct conninfra_ctx *infra_ctx = &g_conninfra_ctx;
...
int conninfra_core_trg_chip_rst(enum consys_drv_type drv, char *reason)
{
	...
	if (snprintf(infra_ctx->trg_reason, CHIP_RST_REASON_MAX_LEN, "%s",
		     reason) < 0)
		pr_warn("[%s::%d] snprintf error\n", __func__, __LINE__);
	ret = msg_thread_send_1(&infra_ctx->cb_ctx, CONNINFRA_CB_OPID_CHIP_RST,
				drv);
	if (ret) {
...
static inline char *
conninfra_core_spi_subsys_string(enum sys_spi_subsystem subsystem)
{
...
int conninfra_core_spi_read(enum sys_spi_subsystem subsystem, unsigned int addr,
			    unsigned int *data)
{
	int ret = 0;
	struct conninfra_ctx *infra_ctx = &g_conninfra_ctx;
	...
	ret = msg_thread_send_wait_3(&infra_ctx->msg_ctx,
				     CONNINFRA_OPID_RFSPI_READ, 0, subsystem,
				     addr, data_ptr);
	if (ret) {
		pr_err("[%s] failed (ret = %d). subsystem=%s addr=%x\n",
		       __func__, ret,
		       conninfra_core_spi_subsys_string(subsystem), addr);
		return CONNINFRA_SPI_OP_FAIL;
	}
	return 0;
}

int conninfra_core_spi_write(enum sys_spi_subsystem subsystem,
			     unsigned int addr, unsigned int data)
{
	int ret;
	ret = msg_thread_send_wait_3(&(g_conninfra_ctx.msg_ctx),
				     CONNINFRA_OPID_RFSPI_WRITE, 0, subsystem,
				     addr, data);
	if (ret) {
		pr_err("[%s] failed (ret = %d). subsystem=%s addr=0x%x data=0x%x\n",
		       __func__, ret,
		       conninfra_core_spi_subsys_string(subsystem), addr, data);
...
int conninfra_core_spi_update_bits(enum sys_spi_subsystem subsystem,
				   unsigned int addr, unsigned int data,
				   unsigned int mask)
{
	int ret;
	ret = msg_thread_send_wait_4(&(g_conninfra_ctx.msg_ctx),
				     CONNINFRA_OPID_RFSPI_UPDATE_BITS, 0,
...
int conninfra_core_adie_top_ck_en_off(enum consys_adie_ctl_type type)
{
	...
	ret = msg_thread_send_wait_1(&infra_ctx->msg_ctx,
				     CONNINFRA_OPID_ADIE_TOP_CK_EN_OFF, 0,
				     type);
	if (ret) {
		pr_err("[%s] fail, ret = %d\n", __func__, ret);
		return -1;
				  ...
				  struct sub_drv_ops_cb *cb)
{
	...
	pr_info("[%s] [pre_cal] type=[%s] cb rst=[%p][%p] pre_cal=[%p][%p], therm=[%p]",
		__func__, drv_name[type], cb->rst_cb.pre_whole_chip_rst,
		cb->rst_cb.post_whole_chip_rst, cb->pre_cal_cb.pwr_on_cb,
		cb->pre_cal_cb.do_cal_cb, cb->thermal_qry);

	pr_info("[%s] [pre_cal] type=[%d] bt=[%p] wf=[%p]", __func__, type,
		infra_ctx->drv_inst[CONNDRV_TYPE_BT].ops_cb.pre_cal_cb.pwr_on_cb,
		infra_ctx->drv_inst[CONNDRV_TYPE_WIFI]
			.ops_cb.pre_cal_cb.pwr_on_cb);

	/* WiFi-only mainline port: WiFi pre-cal must not wait for the optional
	 * Bluetooth driver.  If BT is present, opfunc_pre_cal() will include it;
	 * otherwise the calibration state machine runs with WiFi alone.
	 */
	if (infra_ctx->drv_inst[CONNDRV_TYPE_WIFI].ops_cb.pre_cal_cb.do_cal_cb !=
	    NULL)
		trigger_pre_cal = 1;
	...
	if (trigger_pre_cal) {
		pr_info("[%s] [pre_cal] trigger pre-cal; WiFi registered, BT optional",
			__func__);
		ret = msg_thread_send_1(&infra_ctx->msg_ctx,
					CONNINFRA_OPID_PRE_CAL_PREPARE,
...
static int
conninfra_is_pre_cal_timeout_by_cb_not_registered(struct timespec64 *start)
{
	...
	if (diff > CONNINFRA_MAX_PRE_CAL_BLOCKING_TIME) {
		bt_cb = (void *)infra_ctx->drv_inst[CONNDRV_TYPE_BT]
				.ops_cb.pre_cal_cb.do_cal_cb;
		wifi_cb = (void *)infra_ctx->drv_inst[CONNDRV_TYPE_WIFI]
				  .ops_cb.pre_cal_cb.do_cal_cb;
		if (bt_cb == NULL || wifi_cb == NULL) {
			...
			if (snprintf(
				    exception_log, sizeof(exception_log),
				    "pre-cal timeout. %s callback is not registered",
				    exception_title[exception_title_index]) >
			    0) {
				aed_common_exception_api(
					exception_title[exception_title_index],
					NULL, 0, (const int *)exception_log,
					strlen(exception_log), exception_log,
					0);
			}
#endif
			return 1;
...
#define BLOCKING_CHECK_MONITOR_THREAD 100
	...
	while (true) {
		...
		if (g_pre_cal_mode == PRE_CAL_PWR_ON_DISABLED &&
		    !ever_pre_cal) {
			struct conninfra_ctx *infra_ctx = &g_conninfra_ctx;

			ret = msg_thread_send_1(&infra_ctx->msg_ctx,
						CONNINFRA_OPID_PRE_CAL_PREPARE,
						PRE_CAL_BY_SUBDRV_PWR_ON);
			ever_pre_cal = true;
			pr_info("[%s] [pre_cal] Triggered by subdrv power on and set ever_pre_cal to true, result: %d",
				__func__, ret);
		}
		...
		if (ret) {
			if (cal_info->status == PRE_CAL_NOT_INIT ||
			    cal_info->status == PRE_CAL_SCHEDULED) {
				pr_info("[%s] [pre_cal] ret=[%d] status=[%d]",
					__func__, ret, cal_info->status);
				osal_unlock_sleepable_lock(
					&cal_info->pre_cal_lock);
				if (conninfra_is_pre_cal_timeout_by_cb_not_registered(
					    &start) == 1)
					break;
				osal_sleep_ms(100);
				continue;
			...
			pr_info("[%s] [pre_cal] ret=[%d] status=[%d]", __func__,
				ret, cal_info->status);
			osal_sleep_ms(100);
		}
	}
...
int conninfra_core_dump_power_state(char *buf, unsigned int size)
{
	...
	if (buf && size > 0)
		ret = msg_thread_send_wait_2(&infra_ctx->msg_ctx,
					     CONNINFRA_OPID_DUMP_POWER_STATE, 0,
					     (size_t)buf, size);
	else
		ret = msg_thread_send(&infra_ctx->msg_ctx,
...
int conninfra_core_bus_clock_ctrl(enum consys_drv_type drv_type,
				  unsigned int bus_clock, int status)
{
...
static void conninfra_core_wake_lock_get(void)
{
	...
	pr_info("[%s] after wake_lock(%d)\n", __func__,
		osal_wake_lock_count(&conninfra_wake_lock));
}
...
int conninfra_core_init(void)
{
	...
	pr_info("[%s] [pre_cal] Init g_pre_cal_mode = %u", __func__,
		g_pre_cal_mode);
	...
	INIT_WORK(&infra_ctx->cal_info.pre_cal_work,
		  conninfra_core_pre_cal_work_handler);
