...
{
	/* XAGA: ops table only exists after mtk_conninfra_probe ran */
	if (!consys_hw_ops)
		return 0;
...
int consys_hw_init(struct conninfra_dev_cb *dev_cb)
{
	...
	if (iRet)
		...
		while (atomic_read(&g_hw_init_done) == 0) {
			...
			/*
			 * XAGA bring-up: mtk_conninfra_probe cannot finish
			 * until its M3 dependencies exist (conninfra PMIC/
			 * clocks). Fail the init instead of continuing in a
			 * half-initialized state: conninfra_dev_do_drv_init
			 * handles our error and aborts gracefully. Returning
			 * 0 here made later callers deref the unassigned
			 * consys_hw_ops (0x78 Oops).
			 */
			if (retry >= 200) {
				/*
				 * Return WITHOUT unregistering the driver
				 * here: on module init failure the kernel
				 * calls the module exit fn, which performs
				 * the unregister itself - doing both was a
				 * double-unregister (v31 reboot bug).
				 */
				pr_err("XAGA-CONSYS: hw init not done after %d retries, abort init (M3 pending)\n",
				       retry);
				return -ETIMEDOUT;
			}
		}
	...
	/*
	 * XAGA bring-up: consys_hw_ops is only assigned inside
	 * mtk_conninfra_probe. When the wait above timed out (M3 deps
	 * missing) probe never ran and consys_hw_get_ic_info ->
	 * consys_hw_chipid_get dereferenced the NULL ops table
	 * (+0x78 => virtual address 0x78 Oops). Skip the dependent
	 * calls until M3 makes the probe genuinely succeed.
	 */
	if (!atomic_read(&g_hw_init_done)) {
		pr_err("XAGA-CONSYS: hw never probed, skip conap_scp_init (M3 pending)\n");
	} else {
