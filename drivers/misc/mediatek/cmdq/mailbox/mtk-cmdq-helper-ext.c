...
/*
 * Last errno seen in cmdq_mbox_create, for callers that need to tell
 * -EPROBE_DEFER (retry makes sense) apart from permanent failures.
 */
static int g_cmdq_mbox_last_err;

int cmdq_mbox_last_err(void)
{
	return g_cmdq_mbox_last_err;
}
EXPORT_SYMBOL(cmdq_mbox_last_err);
...
struct cmdq_client *cmdq_mbox_create(struct device *dev, int index)
{
	...
	if (IS_ERR(client->chan)) {
		g_cmdq_mbox_last_err = PTR_ERR(client->chan);
		cmdq_err("channel request fail:%ld idx:%d",
			PTR_ERR(client->chan), index);
		dev_err_ratelimited(dev,
			"GCE-MBOX: request_channel idx=%d failed: %ld\n",
			index, PTR_ERR(client->chan));
		kfree(client);
		return NULL;
	}
	g_cmdq_mbox_last_err = 0;
...
s32 cmdq_pkt_finalize(struct cmdq_pkt *pkt)
{
	...
	/* JUMP to end (match downstream; Android C3D thread22 uses this) */
