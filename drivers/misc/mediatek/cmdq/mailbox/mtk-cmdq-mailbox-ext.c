...
static int cmdq_probe(struct platform_device *pdev)
{
	...
	if (err < 0) {
		...
		dev_err(dev, "XAGA-GCE: mbox_controller_register FAILED:%d\n",
			err);
		return err;
	}
	dev_notice(dev, "register mailbox successfully\n");
	/* visible at loglevel=3: timestamp the moment GCE mbox is usable */
	dev_err(dev, "XAGA-GCE: mailbox registered OK, chans=%d\n",
		cmdq->mbox.num_chans);
