				       ...
				       struct mtk_plane_state *mtk_plane_state)
{
	...
	pr_info("XAGA-PLANE: addr=0x%llx pitch=%d src=(%d,%d) dst=(%d,%d) w=%d h=%d fmt=0x%x mod=0x%llx\n",
		(unsigned long long)addr, pitch, new_state->src.x1 >> 16,
		new_state->src.y1 >> 16, new_state->dst.x1, new_state->dst.y1,
		drm_rect_width(&new_state->dst), drm_rect_height(&new_state->dst),
