...
void mtk_rdma_start(struct device *dev)
{
	...
	/*
	 * MT6895 RDMA0 in DSI-buffer mode: downstream mtk_rdma_start writes
	 * 0x1 to RDMA_MEM_GMC_S4 so mmsys0 considers the DSI ultra signal
	 * (the DSI TX-buffer flow control drives the RDMA request pacing).
	 * Without it the RDMA starves and the DSI underruns after a few
	 * frames. See mtk_disp_rdma.c mtk_rdma_start (MMSYS_MT6895 case).
	 */
	if (rdma->data->dsi_buffer && !strcmp(dev_name(dev), "14006000.disp_rdma"))
		writel_relaxed(0x1, rdma->regs + 0x00ec);

	rdma_update_bits(dev, DISP_REG_RDMA_GLOBAL_CON, RDMA_ENGINE_EN,
			 RDMA_ENGINE_EN);

	/*
	 * XAGA: fix RDMA FIFO_CON + INT_EN once here (start runs once at
	 * hw_init). LK left FIFO bit31 (UNDERFLOW_EN) set (0x8f300000); the
	 * working device has it clear (0x0f300000). And enable the working
	 * INT_EN=0x36.
	 */
	if (dev->of_node && of_device_is_compatible(dev->of_node,
						     "mediatek,mt6895-disp-rdma")) {
		unsigned int reg = readl(rdma->regs + DISP_REG_RDMA_FIFO_CON);

		writel_relaxed(reg & ~RDMA_FIFO_UNDERFLOW_EN,
			       rdma->regs + DISP_REG_RDMA_FIFO_CON);
		writel_relaxed(0x36, rdma->regs + DISP_REG_RDMA_INT_ENABLE);
	}
}
...
{
	/*
	 * XAGA: complete no-op. This runs on EVERY DRM commit; re-writing
	 * any RDMA register mid-stream (SIZE or FIFO_CON) can disturb the
	 * RDMA->DSC frame flow -> DSC ABN_EOF. LK left SIZE0=540/SIZE1=2460
	 * and the working device keeps them. FIFO bit31 + INT_EN are fixed
	 * once in mtk_rdma_start instead.
	 */
}
...
/* mt6895 RDMA fifo = SZ_4K*15 + SZ_256*3 = 62208 bytes (downstream
 * mt6895_rdma_driver_data). The mt8195 value (1920) is smaller than one
 * 1080x4B line so the relay underflows every line -> DSC starves -> DSI
 * INP_UNFINISH -> pipeline stalls after ~61 frames. */
static const struct mtk_disp_rdma_data mt6895_rdma_driver_data_1920 = {
	.fifo_size = SZ_4K * 15 + SZ_256 * 3,
	.dsi_buffer = true,
	.formats = mt8173_formats,
	.num_formats = ARRAY_SIZE(mt8173_formats),
...
static const struct of_device_id mtk_disp_rdma_driver_dt_match[] = {
	...
	{ .compatible = "mediatek,mt6895-disp-rdma",
	  .data = &mt6895_rdma_driver_data_1920},
	{},
};
