...
static void mtk_rdma_start(struct mtk_ddp_comp *comp, struct cmdq_pkt *handle)
{
...

	{
		...
		DDPDBG("XAGA-RDMA GLOBAL_CON=0x%08x SIZE_CON0=0x%08x SIZE_CON1=0x%08x\n",
			readl(baddr + DISP_REG_RDMA_GLOBAL_CON),
			readl(baddr + DISP_REG_RDMA_SIZE_CON_0),
			readl(baddr + DISP_REG_RDMA_SIZE_CON_1));
		DDPDBG("XAGA-RDMA FIFO_CON=0x%08x INT_EN=0x%08x INT_STA=0x%08x\n",
			    ...
			    struct mtk_ddp_config *cfg, struct cmdq_pkt *handle)
{
	...
	if (comp->mtk_crtc->is_dual_pipe)
		...
		w = cfg->w;
	DDPDBG("XAGA-STAGE rdma_config: %s dual_pipe=%d cfg.w=%d -> w=%d h=%d\n",
