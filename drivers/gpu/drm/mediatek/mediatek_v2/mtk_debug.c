...
/*
 * XAGA read-only sampler register offsets. Sources:
 *   DSI   offsets from mtk_dsi.c
 *   DSC   offsets from mtk_disp_dsc.c
 *   RDMA  offsets from mtk_disp_rdma.c
 */
#define XAGA_DSI_INTSTA		0x0c	/* mtk_dsi.c:73   DSI_INTSTA */
#define XAGA_DSI_CON_CTRL	0x10	/* mtk_dsi.c:94   DSI_CON_CTRL */
#define XAGA_DSI_MODE_CTRL	0x14	/* mtk_dsi.c:103  DSI_MODE_CTRL */
#define XAGA_DSI_STATE_DBG7	0x164	/* mtk_dsi.c:267  DSI_STATE_DBG7 (VDO FSM) */
#define XAGA_DSI_LINE_CNT	0x16c	/* mtk_dsi.c:3801 readl(dsi->regs + 0x16C) LINE_COUNTER */
#define XAGA_DSC_CON		0x0000	/* mtk_disp_dsc.c:28 DISP_REG_DSC_CON */
#define XAGA_DSC_INTSTA		0x0008	/* mtk_disp_dsc.c:40 DISP_REG_DSC_INTSTA */
#define XAGA_RDMA_INT_STATUS	0x0004	/* mtk_disp_rdma.c:37 DISP_REG_RDMA_INT_STATUS */
#define XAGA_RDMA_GLOBAL_CON	0x0010	/* mtk_disp_rdma.c:44 DISP_REG_RDMA_GLOBAL_CON */
#define XAGA_RDMA_SIZE_CON_0	0x0014	/* mtk_disp_rdma.c:48 DISP_REG_RDMA_SIZE_CON_0 (width) */
#define XAGA_RDMA_SIZE_CON_1	0x0018	/* mtk_disp_rdma.c:51 DISP_REG_RDMA_SIZE_CON_1 (height) */
#define XAGA_RDMA_MEM_START_ADDR 0x0f00 /* mtk_disp_rdma.c:54 DISP_REG_RDMA_MEM_START_ADDR */
#define XAGA_RDMA_OUT_LINE_CNT	0x012c	/* mtk_disp_rdma.c:137 DISP_REG_RDMA_OUT_LINE_CNT */
#define XAGA_OVL_EN		0x000c	/* mtk_disp_ovl.c:77  DISP_REG_OVL_EN */
#define XAGA_OVL_ROI_SIZE	0x0020	/* mtk_disp_ovl.c:85  DISP_REG_OVL_ROI_SIZE */
#define XAGA_OVL_SRC_SIZE	0x0038	/* mtk_disp_ovl.c:127 DISP_REG_OVL_SRC_SIZE(0) */
#define XAGA_OVL_PITCH		0x0044	/* mtk_disp_ovl.c:133 DISP_REG_OVL_PITCH(0) */
#define XAGA_OVL_RDMA_CTRL	0x00c0	/* mtk_disp_ovl.c:139 DISP_REG_OVL_RDMA_CTRL(0) */
#define XAGA_OVL_ADDR		0x0f40	/* mtk_disp_ovl.c:245 DISP_REG_OVL_ADDR_BASE (layer0) */
#define XAGA_MUTEX_INTSTA	0x0004	/* mtk_drm_ddp.c:143 DISP_REG_MUTEX_INTSTA */
#define XAGA_MUTEX_CFG		0x0008	/* mtk_drm_ddp.c:144 DISP_REG_MUTEX_CFG */
#define XAGA_MUTEX_EN		0x0020	/* mtk_drm_ddp.c:145 DISP_REG_MUTEX_EN(0) */
#define XAGA_MUTEX_STATUS	0x0024	/* mtk_drm_ddp.c:146 DISP_REG_MUTEX(0), BIT1=INT_MUTEX */
#define XAGA_MUTEX_SOF		0x002c	/* mtk_drm_ddp.c:970 MT6895_DISP_MUTEX0_SOF */
#define XAGA_MUTEX_MOD		0x0030	/* mtk_drm_ddp.c:969 MT6895_DISP_MUTEX0_MOD0 (component connect) */
...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "helper", 6) == 0) {
	...
	} else if (strncmp(opt, "kick_mutex", 10) == 0) {
		/*
		 * XAGA DIAG (WORKAROUND, diagnosis only): toggle mutex EN
		 * 0->1 to emit one software frame-start pulse, bypassing the
		 * missing DSI->mutex SOF signal. Purpose: verify the whole
		 * display path can complete one frame at all.
		 */
		struct drm_crtc *crtc;
		struct mtk_drm_crtc *mtk_crtc;
		...
		mtk_crtc = to_mtk_crtc(crtc);
		DDPPR_ERR("XAGA kick_mutex: force trigger (SOF->single + EN pulse)\n");
		mtk_disp_mutex_force_trigger(mtk_crtc->mutex[0]);
	} else if (strncmp(opt, "dsi_vdo", 7) == 0) {
		/*
		 * XAGA DIAG (WORKAROUND, diagnosis only): read DSI MODE_CTRL.
		 * If the DSI is stuck in CMD mode after a GCE doze/backlight
		 * command path wedges mid-way, restore VDO mode + START pulse
		 * so the video engine resumes timing (and SOF for mutex).
		 */
		struct drm_crtc *crtc;
		struct mtk_drm_crtc *mtk_crtc;
		...
		if (!out || !out->regs) {
			DDPPR_ERR("XAGA dsi_vdo: no output comp regs\n");
			return;
		}
		/* DSI register offsets (mtk_dsi.c):
		 * CON_CTRL=0x10, MODE_CTRL=0x14, START=0x00 */
		mode = readl(out->regs + 0x14);  /* DSI_MODE_CTRL */
		start = readl(out->regs + 0x00); /* DSI_START */
		DDPPR_ERR("XAGA dsi_vdo: MODE=0x%x START=0x%x CON=0x%x -> restore EN+VDO+START\n",
			  mode, start, readl(out->regs + 0x10));
		{
			u32 c = readl(out->regs + 0x10); /* DSI_CON_CTRL */
			writel((c | BIT(1)) & ~BIT(24), out->regs + 0x10);
			/* DSI_EN=1, clear CM_MODE_WAIT_DATA_EVERY_LINE */
		}
		writel(1, out->regs + 0x14); /* SYNC_PULSE_MODE */
		writel(0, out->regs + 0x00);
		writel(1, out->regs + 0x00);

		/* open one mutex window so OVL/RDMA push data into DSI */
		mtk_disp_mutex_force_trigger(mtk_crtc->mutex[0]);

		/*
		 * Wake the trigger loop (thr3) which is stuck on
		 * WFE(dsi0_eof): inject the event via a small GCE task so
		 * the loop advances to the next iteration and issues the
		 * frame trigger (START pulse) itself.
		 */
		{
			...
			if (h) {
				...
				cmdq_pkt_flush(h);
				cmdq_pkt_destroy(h);
				DDPPR_ERR("XAGA dsi_vdo: injected dsi0_eof event\n");
			} else {
				DDPPR_ERR("XAGA dsi_vdo: pkt create fail\n");
			}
		}
		 ...
		 * WFE the trigger loop is stuck on. Offsets mirror
		 * drivers/misc/mediatek/cmdq/mailbox/mtk-cmdq-mailbox-ext.c
		 * (CMDQ_THR_BASE 0x100, CMDQ_THR_SIZE 0x80).
		 */
		void __iomem *gce;
		int t;

		gce = ioremap(0x1e980000, 0x4000);
		if (!gce) {
			DDPPR_ERR("XAGA gce_dump: ioremap fail\n");
			return;
		}
		for (t = 0; t < 26; t++) {
			void __iomem *thr = gce + 0x100 + 0x80 * t;
			u32 en = readl(thr + 0x04);
			u32 pc = readl(thr + 0x20);
			u32 end = readl(thr + 0x24);
			u32 cnt = readl(thr + 0x28);
			u32 wait = readl(thr + 0x30);

			if (!en && !pc && !cnt)
				continue;
			DDPPR_ERR("XAGA gce_dump: thr%d EN=0x%x PC=0x%x END=0x%x CNT=0x%x WAIT=0x%x\n",
				  t, en, pc, end, cnt, wait);
			if (en && pc) {
				/* dump instructions at PC (DRAM phys addr,
				 * use phys_to_virt via linear map) */
				u64 *inst = phys_to_virt(pc);
				int k;

				for (k = 0; k < 6; k++)
					DDPPR_ERR("XAGA gce_dump: thr%d inst%d@%llx = 0x%016llx\n",
						  t, k, (u64)pc + k * 8,
						  (u64)readq(inst + k));
			}
		}
	...
	} else if (strncmp(opt, "xaga_regs", 9) == 0) {
		/*
		 * XAGA DIAG (read-only): sample DSI0/DSC0/RDMA0 status
		 * registers over ~60ms to observe whether a frame boundary
		 * is advancing. Pure readl sampling; no register is written
		 * and no pipeline state is changed.
		 */
		struct drm_crtc *crtc;
		struct mtk_drm_crtc *mtk_crtc;
		struct mtk_drm_private *priv;
		struct mtk_ddp_comp *dsi, *dsc, *rdma, *ovl, *ovl2l;
		struct mtk_disp_mutex *mutex;
		struct mtk_ddp *ddp = NULL;
		void __iomem *mregs = NULL;
		int i;

		crtc = list_first_entry(&(drm_dev)->mode_config.crtc_list,
					typeof(*crtc), head);
		if (!crtc) {
			DDPPR_ERR("XAGA xaga_regs: find crtc fail\n");
			return;
		}
		mtk_crtc = to_mtk_crtc(crtc);
		priv = crtc->dev->dev_private;
		dsi = mtk_ddp_comp_request_output(mtk_crtc);
		dsc = priv->ddp_comp[DDP_COMPONENT_DSC0];
		rdma = mtk_ddp_comp_find_by_id(crtc, DDP_COMPONENT_RDMA0);
		ovl = priv->ddp_comp[DDP_COMPONENT_OVL0];
		ovl2l = priv->ddp_comp[DDP_COMPONENT_OVL1_2L];
		...
		DDPPR_ERR("XAGA xaga_regs: dsi=%p dsc=%p rdma=%p ovl=%p ovl2l=%p\n",
			  dsi, dsc, rdma, ovl, ovl2l);
		if (mregs)
			DDPPR_ERR("XAGA xaga_regs: MUTEX regs_pa=0x%llx\n",
				  (unsigned long long)ddp->regs_pa);
		if (dsi && dsi->regs)
			DDPPR_ERR("XAGA xaga_regs: DSI regs_pa=0x%llx\n",
				  (unsigned long long)dsi->regs_pa);
		if (dsc && dsc->regs)
			DDPPR_ERR("XAGA xaga_regs: DSC regs_pa=0x%llx\n",
				  (unsigned long long)dsc->regs_pa);
		if (rdma && rdma->regs)
			DDPPR_ERR("XAGA xaga_regs: RDMA regs_pa=0x%llx\n",
				  (unsigned long long)rdma->regs_pa);
		if (ovl && ovl->regs)
			DDPPR_ERR("XAGA xaga_regs: OVL regs_pa=0x%llx\n",
				  (unsigned long long)ovl->regs_pa);
		if (ovl2l && ovl2l->regs)
			DDPPR_ERR("XAGA xaga_regs: OVL1_2L regs_pa=0x%llx\n",
				  (unsigned long long)ovl2l->regs_pa);

		DDPPR_ERR("XAGA xaga_regs: cmdq_base ovl2l=%p rdma=%p dsi=%p dsc=%p ovl=%p ddp=%p\n",
			  ovl2l ? ovl2l->cmdq_base : NULL,
			  rdma ? rdma->cmdq_base : NULL,
			  dsi ? dsi->cmdq_base : NULL,
			  dsc ? dsc->cmdq_base : NULL,
			  ovl ? ovl->cmdq_base : NULL,
			  ddp ? ddp->cmdq_base : NULL);

		for (i = 0; i < 20; i++) {
			u32 fsm = 0, line = 0, dint = 0, dcon = 0, dmode = 0;
			u32 scon = 0, sint = 0;
			u32 rcon = 0, ris = 0, rline = 0, rsize0 = 0, rsize1 = 0, rmaddr = 0;
			u32 oen = 0, osrc = 0, octrl = 0, oaddr = 0;
			u32 o2en = 0, o2src = 0, o2ctrl = 0, o2addr = 0;
			u32 men = 0, mintsta = 0, mstatus = 0, mcfg = 0, msof = 0, mmod = 0;

			if (dsi && dsi->regs) {
				fsm = readl(dsi->regs + XAGA_DSI_STATE_DBG7);
				line = readl(dsi->regs + XAGA_DSI_LINE_CNT);
				dint = readl(dsi->regs + XAGA_DSI_INTSTA);
				dcon = readl(dsi->regs + XAGA_DSI_CON_CTRL);
				dmode = readl(dsi->regs + XAGA_DSI_MODE_CTRL);
			}
			if (dsc && dsc->regs) {
				scon = readl(dsc->regs + XAGA_DSC_CON);
				sint = readl(dsc->regs + XAGA_DSC_INTSTA);
			}
			if (rdma && rdma->regs) {
				rcon = readl(rdma->regs + XAGA_RDMA_GLOBAL_CON);
				ris = readl(rdma->regs + XAGA_RDMA_INT_STATUS);
				rline = readl(rdma->regs + XAGA_RDMA_OUT_LINE_CNT);
				rsize0 = readl(rdma->regs + XAGA_RDMA_SIZE_CON_0);
				rsize1 = readl(rdma->regs + XAGA_RDMA_SIZE_CON_1);
				rmaddr = readl(rdma->regs + XAGA_RDMA_MEM_START_ADDR);
			}
			if (ovl && ovl->regs) {
				oen = readl(ovl->regs + XAGA_OVL_EN);
				osrc = readl(ovl->regs + XAGA_OVL_SRC_SIZE);
				octrl = readl(ovl->regs + XAGA_OVL_RDMA_CTRL);
				oaddr = readl(ovl->regs + XAGA_OVL_ADDR);
			}
			if (ovl2l && ovl2l->regs) {
				o2en = readl(ovl2l->regs + XAGA_OVL_EN);
				o2src = readl(ovl2l->regs + XAGA_OVL_SRC_SIZE);
				o2ctrl = readl(ovl2l->regs + XAGA_OVL_RDMA_CTRL);
				o2addr = readl(ovl2l->regs + XAGA_OVL_ADDR);
			}
			if (mregs) {
				men = readl(mregs + XAGA_MUTEX_EN);
				mintsta = readl(mregs + XAGA_MUTEX_INTSTA);
				mstatus = readl(mregs + XAGA_MUTEX_STATUS);
				mcfg = readl(mregs + XAGA_MUTEX_CFG);
				msof = readl(mregs + XAGA_MUTEX_SOF);
				mmod = readl(mregs + XAGA_MUTEX_MOD);
			}
			DDPPR_ERR("XAGA xaga_regs[%02d] FSM=0x%02x LINE=0x%05x DINT=0x%08x DCON=0x%08x DMODE=0x%08x SCON=0x%08x SINT=0x%08x RCON=0x%08x RIS=0x%08x RSZ0=0x%05x RSZ1=0x%05x RMADDR=0x%08x RLINE=0x%05x OEN=0x%08x OSRC=0x%08x OCTRL=0x%08x OADDR=0x%08x O2EN=0x%08x O2SRC=0x%08x O2CTRL=0x%08x O2ADDR=0x%08x MEN=0x%08x MINTSTA=0x%08x MSTAT=0x%08x MCFG=0x%08x MSOF=0x%08x MMOD=0x%08x\n",
				  i, fsm, line, dint, dcon, dmode,
				  scon, sint, rcon, ris, rsize0, rsize1, rmaddr, rline,
				  oen, osrc, octrl, oaddr, o2en, o2src, o2ctrl, o2addr,
				  men, mintsta, mstatus, mcfg, msof, mmod);
			usleep_range(3000, 4000);
		}
	} else if (strncmp(opt, "ovl_wr", 6) == 0) {
		/*
		 * XAGA DIAG (read/write readback): probe whether OVL1_2L layer-0
		 * address reg (0x0f40) is a shadow register. Write 0xdeadbeef then
		 * read back: 0xdeadbeef => direct register (config write never
		 * reached it); unchanged => shadow register whose active value only
		 * updates on the SOF commit. Original value restored afterwards.
		 */
		struct drm_crtc *crtc;
		struct mtk_drm_crtc *mtk_crtc;
		struct mtk_drm_private *priv;
		...
		ovl2l = priv->ddp_comp[DDP_COMPONENT_OVL1_2L];
		if (!ovl2l || !ovl2l->regs) {
			DDPPR_ERR("XAGA ovl_wr: no ovl2l regs\n");
			return;
		}
		orig = readl(ovl2l->regs + XAGA_OVL_ADDR);
		writel(0xdeadbeef, ovl2l->regs + XAGA_OVL_ADDR);
		rd = readl(ovl2l->regs + XAGA_OVL_ADDR);
		writel(orig, ovl2l->regs + XAGA_OVL_ADDR);
		DDPPR_ERR("XAGA ovl_wr: orig=0x%08x wrote=0xdeadbeef readback=0x%08x -> %s\n",
			  orig, rd,
			  (rd == 0xdeadbeef) ? "DIRECT register" :
					       "SHADOW register (needs SOF commit)");
