			      ...
			      enum mtk_ddp_comp_id next)
{
	...
	case MMSYS_MT6895:
		...
		if (value >= 0) {
			if (addr == MT6895_DISP_RDMA0_POS_MOUT_EN) {
				u32 mask = MT6895_DISP_RDMA0_POS_MOUT_EN_TO_DISP_RDMA0_SEL |
					    MT6895_DISP_RDMA0_POS_MOUT_EN_TO_DISP_CHIST0_SEL_IN |
					    MT6895_DISP_RDMA0_POS_MOUT_EN_TO_DISP_CHIST1_SEL_IN;
				u32 val = MT6895_DISP_RDMA0_POS_MOUT_EN_TO_DISP_RDMA0_SEL |
					    MT6895_DISP_RDMA0_POS_MOUT_EN_TO_DISP_CHIST1_SEL_IN;
				...
				reg |= val | (val << 16);
				writel_relaxed(reg, config_regs + addr);
			} else if (addr == MT6895_DISP_DITHER0_MOUT_EN) {
				u32 mask = MT6895_DISP_DITHER0_MOUT_EN_TO_DISP_CM0 |
					    MT6895_DISP_DITHER0_MOUT_EN_TO_DISP_CHIST0_SEL_IN |
					    MT6895_DISP_DITHER0_MOUT_EN_TO_DISP_CHIST1_SEL_IN;
				u32 val = MT6895_DISP_DITHER0_MOUT_EN_TO_DISP_CM0 |
					    MT6895_DISP_DITHER0_MOUT_EN_TO_DISP_CHIST0_SEL_IN;
					...
					struct cmdq_pkt *handle)
{
	...
	case MMSYS_MT6895:
		...
		if (value >= 0) {
			...
			/*
			 * LK hands off with CHIST branches left enabled from
			 * its own path. CMDQ write with mask == value only
			 * sets the new branches, so stale MOUT outputs that
			 * are not selected by Linux still backpressure the
			 * pipeline. Cover every output bit in these two
			 * relay registers and mirror the low 16 bits into
			 * the paired high half as the hardware does.
			 */
			if (addr == MT6895_DISP_RDMA0_POS_MOUT_EN) {
				mout_mask = MT6895_DISP_RDMA0_POS_MOUT_EN_TO_DISP_RDMA0_SEL |
					    MT6895_DISP_RDMA0_POS_MOUT_EN_TO_DISP_CHIST0_SEL_IN |
					    MT6895_DISP_RDMA0_POS_MOUT_EN_TO_DISP_CHIST1_SEL_IN;
				value = MT6895_DISP_RDMA0_POS_MOUT_EN_TO_DISP_RDMA0_SEL |
					MT6895_DISP_RDMA0_POS_MOUT_EN_TO_DISP_CHIST1_SEL_IN;
				value |= value << 16;
				mout_mask |= mout_mask << 16;
			} else if (addr == MT6895_DISP_DITHER0_MOUT_EN) {
				mout_mask = MT6895_DISP_DITHER0_MOUT_EN_TO_DISP_CM0 |
					    MT6895_DISP_DITHER0_MOUT_EN_TO_DISP_CHIST0_SEL_IN |
					    MT6895_DISP_DITHER0_MOUT_EN_TO_DISP_CHIST1_SEL_IN;
				value = MT6895_DISP_DITHER0_MOUT_EN_TO_DISP_CM0 |
					MT6895_DISP_DITHER0_MOUT_EN_TO_DISP_CHIST0_SEL_IN;
...
/*
 * XAGA DIAG (WORKAROUND, diagnosis only): switch mutex SOF source to
 * single mode (software driven) then pulse EN 0->1 to open one frame
 * window without the DSI SOF signal. Used to verify whether the display
 * path can complete a frame at all when DSI->mutex SOF is missing.
 */
void mtk_disp_mutex_force_trigger(struct mtk_disp_mutex *mutex)
{
	...
	writel_relaxed(0, ddp->regs + DISP_REG_MUTEX_SOF(ddp->data,
						       mutex->id));
	writel_relaxed(0, ddp->regs + DISP_REG_MUTEX_EN(mutex->id));
	writel_relaxed(1, ddp->regs + DISP_REG_MUTEX_EN(mutex->id));
}
...
void mtk_disp_mutex_acquire(struct mtk_disp_mutex *mutex)
{
	...
	/*
	 * mt6895: LK leaves the mutex running and the DSI generates the SOF
	 * in video mode, so the SW-mutex grant bit (INT_MUTEX) is never
	 * asserted on this path. Both polls therefore time out on every
	 * single commit: measured 13.8 ms each (10 ms nominal + udelay
	 * overshoot) of atomic busy-wait, i.e. 27.6 ms out of a 41.86 ms
	 * frame => 23.9 fps while DSI/RDMA/mutex IRQs all run at a clean
	 * 120 Hz. The MUTEX_EN/MUTEX writes are dropped along with the polls
	 * because re-arming a mutex that is already running can disturb the
	 * in-flight frame sync (DSC ABN_EOF -> panel test pattern), which is
	 * the same reasoning as the mt6895 short-circuit already present in
	 * drivers/soc/mediatek/mtk-mutex.c.
	 */
	if (ddp->data == &mt6895_ddp_driver_data)
		return;
...
void mtk_disp_mutex_release(struct mtk_disp_mutex *mutex)
{
	...
	/* Paired with the acquire short-circuit above. */
	if (ddp->data == &mt6895_ddp_driver_data)
		return;
