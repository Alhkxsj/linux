			   ...
			   struct drm_fb_helper_surface_size *sizes)
{
	...
	/*
	 * Bring-up probe: fill the console fb solid green. If the display
	 * pipeline scans this buffer out, the panel visibly turns green
	 * even with zero console output - separates "scanout dead" from
	 * "nobody drew into the fb".
	 */
	{
		u32 __iomem *p = (u32 __iomem *)info->screen_base;
		size_t n = size / sizeof(u32), k;

		for (k = 0; k < n; k++)
			writel(0xFF00FF00u, p + k);
	}
	dev_err(dev->dev,
		"XAGA-FBDEV: fb %ux%u stride %u size %zu base%s 0x%pa filled green\n",
		mode.width, mode.height, mode.pitches[0], size,
		fb_base ? "(LK videolfb)" : "(fresh)", &fb_base);
