...
static void mtk_ovl_start(struct mtk_ddp_comp *comp, struct cmdq_pkt *handle)
{
	...
	/*
	 * Soft-reset the OVL frame machine before enable so the first SOF is
	 * latched (vendor 5.10 mtk_ovl_start does this here). 6.18 had moved
	 * this reset into mtk_ovl_stop, which left the FME un-armed on the
	 * enable edge -> SOF ignored -> no per-frame read -> underrun.
	 */
	...
	DDPDBG("XAGA-STAGE ovl_start: %s dual_pipe=%d EN=0x%08x SRC_CON=0x%08x DATAPATH=0x%08x ROI=0x%08x\n",
