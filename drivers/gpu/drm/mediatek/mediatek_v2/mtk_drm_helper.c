...
void mtk_drm_helper_init(struct device *dev, struct mtk_drm_helper **helper_opt)
{
	...
	/* XAGA bring-up: 强制关闭 idle manager。
	 * dtb 来自设备 dtbo 分区(非本树), 其 helper-value 里 IDLE_MGR=1,
	 * 于是 mtk_drm_idlemgr_init() 会拉起 mtk_drm_disp_idlemgr-0 线程。
	 * 实测该线程与 userspace 的 drm_mode_getconnector 双双卡死在
	 * drm_modeset_lock (D 状态, 不可 kill), 并会把面板推入 idle 省电态,
	 * 使显示通路时好时坏。bring-up 阶段不需要省电, 先整体关闭。
	 */
	mtk_drm_helper_set_opt_by_name(tmp_opt, "MTK_DRM_OPT_IDLE_MGR", 0);
	mtk_drm_helper_set_opt_by_name(tmp_opt, "MTK_DRM_OPT_SODI_SUPPORT", 0);
