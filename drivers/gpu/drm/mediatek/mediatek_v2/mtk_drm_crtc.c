...
static void mtk_drm_crtc_destroy(struct drm_crtc *crtc)
{
	...
	/*
	 * Release every claimed disp mutex, not just mutex[0]: with
	 * DDP_PATH_NR == 2 the create path claims mutex[0] and [1],
	 * and leaking [1] made any later bind fail with -EBUSY.
	 */
	for (i = 0; i < DDP_PATH_NR; i++) {
		if (!IS_ERR_OR_NULL(mtk_crtc->mutex[i]))
			mtk_disp_mutex_put(mtk_crtc->mutex[i]);
		mtk_crtc->mutex[i] = NULL;
	}
...
int mtk_drm_setbacklight(struct drm_crtc *crtc, unsigned int level)
{
	...
	/*
	 * WORKAROUND (bring-up): in VDO mode do not wait for frame done.
	 * mtk_crtc_wait_frame_done() queues a WFE on EVENT_CMD_EOF and
	 * cmdq_pkt_flush_threaded() then blocks until that EOF arrives; on a
	 * pipeline that has not produced its first frame yet (RDMA address
	 * still 0) this blocks forever while holding mtk_crtc->lock, which
	 * in turn blocks the very atomic commit that would start the frames.
	 * Only CMD/frame-trigger mode needs the frame-boundary sync here.
	 */
	if (is_frame_mode) {
...
bool mtk_crtc_is_dual_pipe(struct drm_crtc *crtc)
{
	struct mtk_panel_params *panel_ext =
		mtk_drm_get_lcm_ext_params(crtc);
	struct mtk_drm_private *priv __maybe_unused = crtc->dev->dev_private;
	...
	if ((drm_crtc_index(crtc) == 0) &&
		...
		/* XAGA: this device's dts ships MTK_DRM_OPT_PRIM_DUAL_PIPE=0,
		 * but the panel is DSC_SINGLE_PORT with slice_mode=1, i.e. two
		 * 540px slices that must be fed by two OVL pipes -- exactly how
		 * LK hands the display over (OVL0 L1 + OVL1_2L L0, both
		 * ROI=0x096c021c). Honouring the opt collapses the path to a
		 * single 1080px pipe while the DSC encoder stays configured for
		 * 540px slices, so the panel receives a malformed stream and
		 * shows white. The panel geometry alone is sufficient here.
		 */
		return true;
	}
...
static bool mtk_crtc_default_layer_valid(struct mtk_ddp_comp *comp,
					 ...
					 struct mtk_plane_state *plane_state)
{
	...
	if (layer_num > 0 && plane_index < layer_num)
		return true;

	/*
	 * WORKAROUND (bring-up): generic KMS clients do not provide the
	 * MediaTek layering blob. In that case comp_id remains zero and the
	 * vendor fallback treats the global DRM plane index as a physical OVL
	 * layer index. Xorg's cursor is plane 11, so this generates register
	 * writes beyond the OVL's 4 KiB resource and into the following block.
	 * Skip unsupported unassigned planes until a complete fallback layer
	 * allocator is implemented.
	 */
	pr_err_ratelimited("XAGA-DUAL: skip unassigned plane=%u on comp=%d (layers=%d)\n",
			   plane_index, comp->id, layer_num);
	return false;
}
...
static void mtk_crtc_ddp_config(struct drm_crtc *crtc)
{
	...
	for (i = 0; i < mtk_crtc->layer_nr; i++) {
		...
		/* dual pipe 下 comp 是左半 OVL, 直接写未拆分的全宽 plane_state
		 * 会把 dual_layer_config 算好的半宽覆盖掉 (ROI=540 / layer=1080
		 * 矛盾 -> OVL 不输出)。此处必须同样走左右拆分。
		 */
		if (!mtk_crtc_default_layer_valid(comp, i, plane_state)) {
			continue;
		} else if (mtk_crtc->is_dual_pipe) {
			static int ddpcfg_dbg;
			...
			mtk_crtc_dual_layer_config(mtk_crtc, comp, i,
						   plane_state, cmdq_handle);
		} else {
		...
		struct mtk_plane_state *plane_state, struct cmdq_pkt *cmdq_handle)
{
	...
	DDPDBG("XAGA-DUAL[cfg] idx=%d src=%d -> R:comp=%d Rw=%u Rx=%u | L:comp=%d Lw=%u Lx=%u\n",
	       ...
	       comp->id,
	       plane_state_l.pending.width, plane_state_l.pending.dst_x);
...
/* 按 plane 全局 index 计算其归属的 OVL 组件及组件内局部层号。
 * 映射规则与 fake_layer 一致: 先 OVL*_2L 的前 layer_num 层, 余下归主 OVL。
 * 无可归属层时返回 NULL。
 */
static int mtk_drm_crtc_find_ovl_comp_id(struct mtk_drm_private *priv,
					 int is_ovl_2l, int is_dual_pipe);

static struct mtk_ddp_comp *mtk_crtc_get_ovl_by_plane_idx(
	struct mtk_drm_crtc *mtk_crtc, unsigned int plane_idx,
	unsigned int *lye_idx)
{
	struct mtk_drm_private *priv = mtk_crtc->base.dev->dev_private;
	int comp_id, layer_num;

	comp_id = mtk_drm_crtc_find_ovl_comp_id(priv, 1, 0);
	if (comp_id < 0)
		comp_id = DDP_COMPONENT_OVL0_2L;
	layer_num = mtk_ovl_layer_num(priv->ddp_comp[comp_id]);
	if (layer_num > 0) {
		if (plane_idx < (unsigned int)layer_num) {
			*lye_idx = plane_idx;
			return priv->ddp_comp[comp_id];
		}
		plane_idx -= layer_num;
	}

	comp_id = mtk_drm_crtc_find_ovl_comp_id(priv, 0, 0);
	if (comp_id < 0)
		comp_id = DDP_COMPONENT_OVL0;
	layer_num = mtk_ovl_layer_num(priv->ddp_comp[comp_id]);
	if (layer_num <= 0 || plane_idx >= (unsigned int)layer_num)
		return NULL;
	*lye_idx = plane_idx;
	return priv->ddp_comp[comp_id];
}
...
void mtk_crtc_restore_plane_setting(struct mtk_drm_crtc *mtk_crtc)
{
	...
	for (i = 0; i < mtk_crtc->layer_nr; i++) {
		...
		if (plane_state->comp_state.comp_id) {
			...
			lye_idx = plane_state->comp_state.lye_id;
		} else {
			/* 未被动态分配的 plane 按 index 映射到正确的 OVL
			 * 组件与局部层号, 使接管时能覆盖路径内全部 OVL。
			 * 此前一律默认首组件(OVL*_2L), 主 OVL 的 LK 残留层
			 * 从未被管理 -> IOMMU fault -> DSI underrun。
			 */
			comp = mtk_crtc_get_ovl_by_plane_idx(mtk_crtc, i,
							     &lye_idx);
		}
		...
		if (mtk_crtc->is_dual_pipe) {
			mtk_crtc_dual_layer_config(mtk_crtc, comp, lye_idx,
						   plane_state, cmdq_handle);
		} else {
			mtk_ddp_comp_layer_config(comp, lye_idx, plane_state,
						  cmdq_handle);
		}
...
/* No longer called: see the rationale in mtk_crtc_first_enable_ddp_config(). */
static void __maybe_unused mtk_crtc_all_layer_off(struct mtk_drm_crtc *mtk_crtc,
				   struct cmdq_pkt *cmdq_handle)
{
...
void mtk_crtc_first_enable_ddp_config(struct mtk_drm_crtc *mtk_crtc)
{
	...
	/*
	 * 1. Vendor calls mtk_crtc_all_layer_off() here to leave only the LK
	 * logo on screen. That helper keys off physical layer 0
	 * (SRC_CON & 0x1) and, when it is off, forces the whole OVL into
	 * DISP_OVL_FORCE_RELAY_MODE and zeroes RDMA_CTRL on all four layers.
	 *
	 * This device's LK does not use layer 0: measured at boot, OVL0 has
	 * SRC_CON=0x00000100 (bit 8 = FORCE_RELAY, bit 0 clear) with the live
	 * scanout on layer 1 (CON=0x000021ff, PITCH=0x10e0 = 1080*4,
	 * ADDR=0xfea18000, inside the identity-mapped LK framebuffer region).
	 * So keep_first_layer keeps nothing here and the helper clears the
	 * fetch enable of the only layer that had valid data, which shows up as
	 * "frame underflow!" on every OVL and a black screen until the first
	 * client does a full modeset.
	 *
	 * The point of this path is to inherit LK's scanout untouched, so do
	 * not reprogram the layers at all. Clients that come later run the
	 * complete mtk_drm_crtc_enable() sequence and configure their own.
	 */
...
void mtk_drm_crtc_first_enable(struct drm_crtc *crtc)
{
	...
	/*
	 * XAGA-FE[nn] markers (v211 diagnostic): v210 called this from
	 * mtk_drm_kms_init() and the device hung before USB gadget/SSH came
	 * up, leaving no way to see where. The call now runs from a delayed
	 * work, so these markers plus /proc/<kworker>/stack identify the
	 * blocking step. Remove once the blocker is fixed.
	 */
	/*
	 * Dump the datapath before anything in this function touches it. Every
	 * other dump site runs after mtk_crtc_first_enable_ddp_config(), i.e.
	 * after mtk_crtc_all_layer_off() has already rewritten SRC_CON and the
	 * per-layer RDMA_CTRL, so LK's original scanout configuration was only
	 * ever inferred. This makes it directly observable.
	 */
	...
	if (disp_helper_get_stage() == DISP_HELPER_STAGE_NORMAL) {
		 ...
		 * first-enable path leaves out because it assumes LK handed
		 * over a live datapath that only needs topping up. This
		 * bring-up starts clk_refcnt at 0 (see the comment in
		 * mtk_dsi_probe()), so the first mtk_dsi_poweron() resets the
		 * DSI/PHY and the hardware MUX has to be wired again.
		 *
		 * It must run after mtk_crtc_ddp_prepare() above: connecting
		 * the MUX and enabling the mutex needs the DSI engine and
		 * digital clocks live. The software-side
		 * mtk_crtc_attach_ddp_comp() done in mtk_drm_crtc_init_para()
		 * only keeps bookkeeping and is not enough.
		 *
		 * Steps 6 (config_default_path) and 8 (restore_plane_setting)
		 * are deliberately NOT called here. Both were tried and both
		 * made the first frame worse, because they overwrite the LK
		 * scanout state that mtk_crtc_all_layer_off(keep_first_layer)
		 * in mtk_crtc_first_enable_ddp_config() went out of its way to
		 * preserve. Measured on this panel: OVL1_2L L0 still carried
		 * LK's CON=0x000021ff / PITCH=0x1100 / ADDR=0xfe000000 after
		 * first_enable_ddp_config, and restore_plane_setting then
		 * pushed the empty plane state (pending.addr == 0, no client
		 * has committed a framebuffer this early) over it, leaving
		 * CON=0x10000000 / ADDR=0 -> "frame underflow! cnt=0" on every
		 * OVL and a black screen until the first real atomic commit.
		 *
		 * Step 7 (connect_addon_module) is skipped as well: no addon
				      ...
				      struct drm_atomic_state *atomic_state)
{
	...
	/*
	 * Msync2.0 runtime-fps block. DISABLED for qqcandy bring-up: gcc16 fails
	 * this TU with "-mgeneral-regs-only is incompatible with the use of
	 * floating-point types" attributed here even though the expanded GIMPLE
	 * contains zero FP (forensics in PORTING_SPEC.md TODO). Panel still boots
	 * at its default rate; re-enable after root-causing.
	 */
	#ifdef CONFIG_MTK_CRTC_MSYNC2
			       ...
			       struct mtk_plane_state *plane_state)
{
	...
	if (plane_state->pending.enable) {
		...
		if (!mtk_crtc_default_layer_valid(comp, plane_index,
						  plane_state)) {
			goto update_fences;
		} else if (mtk_crtc->is_dual_pipe) {
			       ...
			       struct mtk_plane_state *plane_state)
{
	...
	DDPDBG("XAGA plane_update: plane=%d en=%d skip=%d comp_id=%d lye=%d ext_lye=%d addr=0x%llx %ux%u fmt=0x%x compress=%d\n",
		...
		plane_state->comp_state.ext_lye_id,
		(unsigned long long)plane_state->pending.addr,
		plane_state->pending.width, plane_state->pending.height,
		plane_state->pending.format,
		(int)plane_state->pending.prop_val[PLANE_PROP_COMPRESS]);
	...
	void *cb_data, struct cmdq_pkt *cmdq_handle)
{
	...
	/* XAGA readback: did the OVL layer-0 addr write actually land? */
	{
		struct mtk_drm_private *priv_rb = crtc->dev->dev_private;
		struct mtk_ddp_comp *ovl_rb =
		...
		if (ovl_rb && ovl_rb->regs)
			DDPDBG("XAGA flush_readback: OVL1_2L ADDR=0x%08x MSB=0x%08x HDR=0x%08x ELADDR=0x%08x ELHDR=0x%08x SRC=0x%08x EN=0x%08x\n",
				...
				readl(ovl_rb->regs + 0x0fb0),   /* EL_ADDR(0) */
				readl(ovl_rb->regs + 0x0fb4),   /* EL_HDR_ADDR(0) */
				readl(ovl_rb->regs + 0x0038),   /* SRC_SIZE(0) */
				readl(ovl_rb->regs + 0x000c));  /* EN */
	}
...
static int mtk_crtc_init_gce_obj(struct drm_device *drm_dev,
				 struct mtk_drm_crtc *mtk_crtc)
{
	...
	for (i = 0; i < CLIENT_TYPE_MAX; i++) {
		...
		if (index < 0) {
			/* name absent from DT: client legitimately unused */
			dev_err(dev,
				"GCE-INIT: crtc%d %s: no DT match, NULL\n",
				drm_crtc_index(&mtk_crtc->base), buf);
			mtk_crtc->gce_obj.client[i] = NULL;
			if (i == CLIENT_CFG) {
				/*
				 * first_enable hard-requires CLIENT_CFG.
				 * Defer instead of building a crtc that
				 * will oops later.
				 */
				dev_err(dev,
					"GCE-INIT: crtc%d CLIENT_CFG missing from DT, deferring\n",
					drm_crtc_index(&mtk_crtc->base));
				return -EPROBE_DEFER;
			}
			continue;
		...
		if (IS_ERR_OR_NULL(mtk_crtc->gce_obj.client[i])) {
			int lerr = cmdq_mbox_last_err();
			/*
			 * Named client but no channel. Release what we
			 * already hold, then either defer the whole bind
			 * (-EPROBE_DEFER: GCE not probed yet) or stop for
			 * good (any other errno: config/hw problem — retry
			 * would just flood the console forever).
			 */
			dev_err(dev,
				"GCE-INIT: crtc%d %s: mbox_create(idx=%d) FAIL err=%d %s\n",
				drm_crtc_index(&mtk_crtc->base), buf, index,
				lerr, lerr == -EPROBE_DEFER ? "(defer)" :
							      "(PERMANENT)");
			mtk_crtc->gce_obj.client[i] = NULL;
			while (--i >= 0) {
				if (!IS_ERR_OR_NULL(mtk_crtc->gce_obj.client[i])) {
			...
			if (lerr == -EPROBE_DEFER)
				return -EPROBE_DEFER;
			return lerr ? : -ENXIO;
		}
		dev_err(dev, "GCE-INIT: crtc%d %s: idx=%d OK\n",
			drm_crtc_index(&mtk_crtc->base), buf, index);
	}
	...
	if (!cmdq_buf->va_base) {
		...
		dev_err(dev, "GCE-INIT: crtc%d cmdq buf alloc FAIL, deferring\n",
			drm_crtc_index(&mtk_crtc->base));
		/*
		 * Channels are exclusively allocated; free them so the
		 * retried bind can request them again (else -EBUSY).
		 */
		for (i = 0; i < CLIENT_TYPE_MAX; i++) {
			if (!IS_ERR_OR_NULL(mtk_crtc->gce_obj.client[i]))
				cmdq_mbox_destroy(mtk_crtc->gce_obj.client[i]);
			mtk_crtc->gce_obj.client[i] = NULL;
		}
		return -EPROBE_DEFER;
	}
			...
			const struct mtk_crtc_path_data *path_data)
{
	...
	ret = mtk_crtc_init_gce_obj(drm_dev, mtk_crtc);
	if (ret) {
		/*
		 * GCE clients not available (usually GCE mbox controller
		 * not probed yet). Unwind this crtc so the bind can be
		 * deferred and retried cleanly: priv state must look as
		 * if this crtc was never created. Mutexes are released
		 * via err_release_mutex; the registered crtc itself is
		 * torn down by drm_mode_config_cleanup in kms_init.
		 */
		dev_err(dev,
			"crtc%d: init_gce_obj failed %d, deferring bind\n",
			pipe, ret);
		priv->crtc[pipe] = NULL;
		priv->num_pipes--;
		goto err_release_mutex;
...
err_release_mutex:
	/*
	 * Every failure after acquiring the disp mutex handles lands here.
	 * Without this, an aborted bind (e.g. -EPROBE_DEFER on GCE) leaked
	 * the claimed mutexes and every retry failed with -EBUSY, flooding
	 * the console in a probe loop.
	 */
	for (i = 0; i < DDP_PATH_NR; i++) {
		if (!IS_ERR_OR_NULL(mtk_crtc->mutex[i]))
			mtk_disp_mutex_put(mtk_crtc->mutex[i]);
