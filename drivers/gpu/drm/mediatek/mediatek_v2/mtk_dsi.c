				    ...
				    u32 before, u32 after)
{
	...
	pr_err("XAGA-RSTW: CON_CTRL mask=0x%08x data=0x%08x  0x%08x -> 0x%08x  (RESET %d->%d)\n",
...
static void mtk_dsi_enable(struct mtk_dsi *dsi)
{
	...
	if (!(readl(dsi->regs + DSI_CON_CTRL) & DSI_EN))
		pr_err("XAGA-DSI[v145probe] enable FAILED: hs=%lu eng=%lu dig=%lu STATE160=0x%08x STATE164=0x%08x\n",
	...
	if (!(readl(dsi->regs + DSI_CON_CTRL) & DSI_EN)) {
		...
		mtk_dsi_mask(dsi, DSI_CON_CTRL, DSI_EN, DSI_EN);
		pr_err("XAGA-DSI[v148retry] cleared CON_CTRL 0x%08x -> wrote DSI_EN -> now 0x%08x (%s)\n",
	...
	if (!(readl(dsi->regs + DSI_CON_CTRL) & DSI_EN)) {
		...
		pr_err("XAGA-DSI[v152start] START was 0x%08x -> cleared -> CON=0x%08x (%s)\n",
...
static void mtk_dsi_encoder_disable(struct drm_encoder *encoder)
{
	...
	DDPINFO("%s\n", __func__);
	pr_err("XAGA-ENC[disable] idx=%d output_en=%d\n", index, dsi->output_en);
...
static void mtk_dsi_encoder_enable(struct drm_encoder *encoder)
{
	...
	DDPINFO("%s\n", __func__);
	pr_err("XAGA-ENC[enable] idx=%d output_en=%d\n", index, dsi->output_en);
				     ...
				     const struct mipi_dsi_msg *msg, u8 flag)
{
	 ...
	 */
	pr_err("XAGA-VMCMD timeout: type=0x%02x len=%zu | INTSTA=0x%08x INTEN=0x%08x START=0x%08x CON=0x%08x MODE=0x%08x\n",
	       msg->type, msg->tx_len,
	       readl(dsi->regs + DSI_INTSTA), readl(dsi->regs + DSI_INTEN),
	       readl(dsi->regs + DSI_START), readl(dsi->regs + DSI_CON_CTRL),
	       readl(dsi->regs + DSI_MODE_CTRL));
	pr_err("XAGA-VMCMD timeout: STATE160=0x%08x DBG7(164)=0x%08x DBG8(168)=0x%08x DBG9(16c)=0x%08x VACT=0x%08x\n",
	       readl(dsi->regs + 0x160), readl(dsi->regs + 0x164),
