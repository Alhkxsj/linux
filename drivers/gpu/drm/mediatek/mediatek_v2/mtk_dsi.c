...
/* v147 probe: DSI_RESET(BIT0) of CON_CTRL reads back as a permanent 1 on this
 * device, which keeps the DSI engine held in reset (INTSTA==0, no pixels).
 * Log every CPU-side write that touches that bit, plus a stack trace, so the
 * writer that leaves it asserted can be identified.
 */
static void xaga_dsi_con_ctrl_probe(u32 offset, u32 mask, u32 data,
				    u32 before, u32 after)
{
	if (offset != DSI_CON_CTRL || !(mask & DSI_RESET))
		return;

	DDPDBG("XAGA-RSTW: CON_CTRL mask=0x%08x data=0x%08x  0x%08x -> 0x%08x  (RESET %d->%d)\n",
	       mask, data, before, after,
	       !!(before & DSI_RESET), !!(after & DSI_RESET));
}

static void mtk_dsi_mask(struct mtk_dsi *dsi, u32 offset, u32 mask, u32 data)
{
	u32 temp = readl(dsi->regs + offset);
	u32 val = (temp & ~mask) | (data & mask);

	writel(val, dsi->regs + offset);

	xaga_dsi_con_ctrl_probe(offset, mask, data, temp,
...
{
	DDPDBG("XAGA-DSI[enable+] CON=0x%08x\n", readl(dsi->regs + DSI_CON_CTRL));
	mtk_dsi_mask(dsi, DSI_CON_CTRL, DSI_EN, DSI_EN);
	if (dsi->driver_data->need_wait_fifo)
		mtk_dsi_mask(dsi, DSI_CON_CTRL, DSI_CM_WAIT_FIFO_FULL_EN,
			DSI_CM_WAIT_FIFO_FULL_EN);
	DDPDBG("XAGA-DSI[enable-] CON=0x%08x\n", readl(dsi->regs + DSI_CON_CTRL));
	/* v145: if DSI_EN did not latch, report the surrounding clock/state
	 * context so the gating source can be narrowed down.
	 */
	if (!(readl(dsi->regs + DSI_CON_CTRL) & DSI_EN))
		DDPDBG("XAGA-DSI[v145probe] enable FAILED: hs=%lu eng=%lu dig=%lu STATE160=0x%08x STATE164=0x%08x\n",
		       clk_get_rate(dsi->hs_clk),
		       clk_get_rate(dsi->engine_clk),
		       clk_get_rate(dsi->digital_clk),
		       readl(dsi->regs + 0x160), readl(dsi->regs + 0x164));

	/* v148 experiment: DSI_EN refuses to latch at boot while CON_CTRL still
	 * carries LK's CM-mode leftovers (observed 0x01000000 = BIT24
	 * DSI_CM_MODE_WAIT_DATA_EVERY_LINE_EN) even though this panel runs VDO
	 * mode. Retry once from a clean CON_CTRL and report whether that is
	 * what blocks the write. Diagnostic only: if the retry is what makes it
	 * stick, the real fix belongs in the LK-handover path.
	 */
	if (!(readl(dsi->regs + DSI_CON_CTRL) & DSI_EN)) {
		u32 before = readl(dsi->regs + DSI_CON_CTRL);

		writel(0, dsi->regs + DSI_CON_CTRL);
		mtk_dsi_mask(dsi, DSI_CON_CTRL, DSI_EN, DSI_EN);
		DDPDBG("XAGA-DSI[v148retry] cleared CON_CTRL 0x%08x -> wrote DSI_EN -> now 0x%08x (%s)\n",
		       before, readl(dsi->regs + DSI_CON_CTRL),
		       (readl(dsi->regs + DSI_CON_CTRL) & DSI_EN) ?
		       "LATCHED" : "still refused");
	}

	/* v151: DSI_READ_WORKING (SHADOW_DEBUG BIT2) is defined but never used
	 * anywhere in the tree. It selects whether readl() returns the shadow
	 * copy or the live working register. Every "DSI_EN refuses to latch"
	 * observation so far has relied on a readback that may have been
	 * looking at the shadow copy the whole time -- while DCS transfers
	 * succeed and the panel visibly reacts, which cannot be true of a
	 * genuinely disabled DSI. Sample CON_CTRL through both views once so
	 * the readback itself stops being a source of false conclusions.
	 */
	/* v152: DSI_EN refuses to latch while the engine is still running.
	 * At boot DSI_START is already 1 (LK left the engine started), which
	 * is consistent with DCS transfers succeeding and the panel visibly
	 * reacting even though DSI_EN reads back as 0: the controller locks
	 * the enable bit while it is started. Quiesce DSI_START first, then
	 * retry the enable, and report whether that is what unblocks it.
	 */
	if (!(readl(dsi->regs + DSI_CON_CTRL) & DSI_EN)) {
		u32 start_before = readl(dsi->regs + DSI_START);
		u32 con_after;

		writel(0, dsi->regs + DSI_START);
		mtk_dsi_mask(dsi, DSI_CON_CTRL, DSI_EN, DSI_EN);
		con_after = readl(dsi->regs + DSI_CON_CTRL);

		DDPDBG("XAGA-DSI[v152start] START was 0x%08x -> cleared -> CON=0x%08x (%s)\n",
		       start_before, con_after,
		       (con_after & DSI_EN) ? "LATCHED" : "still refused");
	}
}
	...
	void *handle, int en)
{
	...
	/*
	 * drm_encoder.crtc is a legacy field, only filled in by
	 * drm_atomic_helper_update_legacy_modeset_state() from a real atomic
	 * commit. mtk_drm_crtc_first_enable() -> mtk_crtc_ddp_prepare() ->
	 * mtk_dsi_poweron() runs before any commit has happened, so it is
	 * still NULL there and dereferencing crtc->state oopses.
	 *
	 * The vendor tree never hit this because it pre-acquires the DSI
	 * clocks in probe (clk_refcnt = 1), which makes the first
	 * mtk_dsi_poweron() bail out at "++clk_refcnt != 1" long before
	 * reaching here. This bring-up deliberately starts the refcount at 0
	 * so the first poweron programs the full data-rate/PHY sequence (see
	 * the comment in mtk_dsi_probe()), which exposes the deref.
	 *
	 * LFR is a low-refresh-rate power optimisation and has nothing to do
	 * with taking the pipe over from LK, so skip it until a CRTC is
	 * actually bound. The normal atomic path still configures it.
	 */
	if (!crtc || !crtc->state) {
		DDPINFO("%s: no crtc bound yet, skipping LFR setup\n", __func__);
		return -1;
	}
...
static int mtk_dsi_set_data_rate(struct mtk_dsi *dsi)
{
	...
	DDPDBG("XAGA-DSI[datarate] set hs_clk to %lu Hz (%d MHz)\n",
	       mipi_tx_rate, data_rate);
	ret = clk_set_rate(dsi->hs_clk, mipi_tx_rate);
	DDPDBG("XAGA-DSI[datarate] clk_set_rate ret=%d\n", ret);
	return ret;
}
...
static int mtk_dsi_poweron(struct mtk_dsi *dsi)
{
	...
	DDPDBG("%s+\n", __func__);
	{
		...
		DDPDBG("XAGA-DSI[poweron+] CON=0x%08x START=0x%08x INTSTA=0x%08x INTEN=0x%08x MODE=0x%08x\n",
		...
		DDPDBG("XAGA-MIPITX[poweron+] PLL_CON0=0x%08x PLL_CON1=0x%08x PLL_CON4=0x%08x LANE_CON=0x%08x VOLTAGE_SEL=0x%08x\n",
	...
	/* 5.10 qqcandy: panel bias/regulator power-on before DSI PHY power */
	...
	if (disp_helper_get_stage() == DISP_HELPER_STAGE_NORMAL) {
		...
		DDPDBG("XAGA-DSI[poweron] phy_power_on done, data_rate=%d MHz\n",
		       dsi->data_rate);
		{
			void __iomem *tx = ioremap(0x11f70000, 0x1000);

			DDPDBG("XAGA-MIPITX[after-phy] PLL_CON0=0x%08x PLL_CON1=0x%08x PLL_CON4=0x%08x LANE_CON=0x%08x VOLTAGE_SEL=0x%08x\n",
	...
	*/
	...
	DDPDBG("XAGA-DSI[poweron-] CON=0x%08x START=0x%08x INTSTA=0x%08x INTEN=0x%08x\n",
	       readl(dsi->regs + DSI_CON_CTRL), readl(dsi->regs + DSI_START),
	       readl(dsi->regs + DSI_INTSTA), readl(dsi->regs + DSI_INTEN));
...
static void mtk_dsi_tx_buf_rw(struct mtk_dsi *dsi)
{
	...
	/* Align with vendor 5.10: fill_rate must scale by ps_wc/width.
	 * 6.18 hardcoded *3 (non-DSC bpp) which is 3x wrong for DSC panels
	 * (ps_wc == width), corrupting the SODI FIFO threshold and stalling
	 * the DSI engine in the line loop. */
...
{
	DDPDBG("XAGA-STAGE dsi_start: START 0->1 (was CON=0x%08x MODE=0x%08x)\n",
	       readl(dsi->regs + DSI_CON_CTRL),
	       readl(dsi->regs + DSI_MODE_CTRL));

	writel(0, dsi->regs + DSI_START);
	writel(1, dsi->regs + DSI_START);
	DDPDBG("XAGA-STAGE dsi_start done: START=0x%08x INTSTA=0x%08x\n",
...
static void mtk_dsi_set_interrupt_enable(struct mtk_dsi *dsi)
{
	...
	DDPDBG("XAGA-DSI[irq_enable] INTSTA(was)=0x%08x -> INTEN=0x%08x\n",
...
static irqreturn_t mtk_dsi_irq_status(int irq, void *dev_id)
{
	...
	/* v147 probe: report CON_CTRL/RESET state from inside the IRQ so a CMDQ
	 * writer (invisible to the CPU-side mtk_dsi_mask hook) can be spotted.
	 * Rate limited to once per second to avoid drowning the log.
	 */
	if (dsi->ddp_comp.id == DDP_COMPONENT_DSI0) {
		static DEFINE_RATELIMIT_STATE(rstw_rs, 1 * HZ, 1);
...
static void mtk_dsi_poweroff(struct mtk_dsi *dsi)
{
	...
	/* 5.10 qqcandy: panel bias power-off after DSI PHY off */
...
static int mtk_preconfig_dsi_enable(struct mtk_dsi *dsi)
{
	...
	DDPDBG("XAGA-DSI[preconfig+] CON=0x%08x START=0x%08x INTSTA=0x%08x\n",
	       readl(dsi->regs + DSI_CON_CTRL), readl(dsi->regs + DSI_START),
	...
	if (dsi->ext && dsi->ext->funcs && dsi->ext->funcs->reset)
		dsi->ext->funcs->reset(dsi->panel, 1);
	mtk_dsi_clk_hs_mode(dsi, 0);
#endif

	DDPDBG("XAGA-STAGE preconfig_dsi_enable done (timing configured)\n");
	xaga_dump_dsi();
	...
	int force_lcm_update)
{
	...
	DDPDBG("XAGA-DSI[output_enable] output_en=%d doze=%d panel=%p\n",
	...
	DDPDBG("XAGA-STAGE output_dsi_enable: panel_prepare + set_mode + hs done\n");
	xaga_dump_dsi();
	...
	if (dsi->panel) {
		...
		DDPDBG("XAGA-DSC[after_panel_enable] %s\n", __func__);
...
static void mtk_dsi_encoder_disable(struct drm_encoder *encoder)
{
	...
	DDPINFO("%s\n", __func__);
	DDPDBG("XAGA-ENC[disable] idx=%d output_en=%d\n", index, dsi->output_en);
...
static void mtk_dsi_encoder_enable(struct drm_encoder *encoder)
{
	...
	DDPINFO("%s\n", __func__);
	DDPDBG("XAGA-ENC[enable] idx=%d output_en=%d\n", index, dsi->output_en);
...
static int mtk_dsi_start_vdo_mode(struct mtk_ddp_comp *comp, void *handle)
{
	...
	DDPDBG("XAGA-DSI[start_vdo] vid_mode=%d\n", vid_mode);
...
static void mtk_dsi_ddp_prepare(struct mtk_ddp_comp *comp)
{
	 ...
	 */
	writel(0, dsi->regs + DSI_INTSTA);
	DDPDBG("XAGA-DSI[ddp_prepare] after direct INTSTA clear=0x%08x\n",
	       readl(dsi->regs + DSI_INTSTA));

	{
		...
		DDPDBG("XAGA-DSI[ddp_prepare] is_dual_pipe=%d crtc=%p slave_dsi=%p\n",
		       mtk_crtc ? mtk_crtc->is_dual_pipe : -1, mtk_crtc,
		       dsi->slave_dsi);
		DDPDBG("XAGA-DSI[ddp_prepare] START=0x%08x CON=0x%08x MODE=0x%08x TXRX=0x%08x\n",
		       readl(r + 0x00), readl(r + DSI_CON_CTRL),
		       readl(r + DSI_MODE_CTRL), readl(r + DSI_TXRX_CTRL));
		DDPDBG("XAGA-DSI[ddp_prepare] PSCTRL=0x%08x SIZE_CON=0x%08x VM_CMD=0x%08x\n",
		       readl(r + DSI_PSCTRL), readl(r + DSI_SIZE_CON),
		       readl(r + dsi->driver_data->reg_vm_cmd_con_ofs));
		DDPDBG("XAGA-DSI[ddp_prepare] LCCON=0x%08x LD0CON=0x%08x BUF_CON1=0x%08x\n",
		       readl(r + DSI_PHY_LCCON), readl(r + DSI_PHY_LD0CON),
		       readl(r + DSI_BUF_CON1));
		DDPDBG("XAGA-DSI[ddp_prepare] VSA=0x%08x VBP=0x%08x VFP=0x%08x VACT=0x%08x\n",
		       readl(r + DSI_VSA_NL), readl(r + DSI_VBP_NL),
		       readl(r + DSI_VFP_NL), readl(r + DSI_VACT_NL));
		DDPDBG("XAGA-DSI[ddp_prepare] HSA=0x%08x HBP=0x%08x HFP=0x%08x HSTX_CKL=0x%08x\n",
		       ...
		       readl(r + DSI_HFP_WC), readl(r + DSI_HSTX_CKL_WC));
		{
			...
			DDPDBG("XAGA-RDMA[LK] GLOBAL_CON=0x%08x SIZE0=0x%08x SIZE1=0x%08x FIFO=0x%08x\n",
			       readl(rr + 0x10), readl(rr + 0x14), readl(rr + 0x18),
			       readl(rr + 0x40));
			DDPDBG("XAGA-RDMA1[LK] GLOBAL_CON=0x%08x SIZE0=0x%08x SIZE1=0x%08x FIFO=0x%08x\n",
			       readl(rr1 + 0x10), readl(rr1 + 0x14), readl(rr1 + 0x18),
			       readl(rr1 + 0x40));
			for (i = 0; i < 3; i++) {
				DDPDBG("XAGA-OVL[%s] EN=0x%08x INTSTA=0x%08x ROI=0x%08x SRC_CON=0x%08x DATAPATH=0x%08x\n",
				       oname[i], readl(or[i] + 0x0c), readl(or[i] + 0x08),
				       readl(or[i] + 0x20), readl(or[i] + 0x2c),
				       readl(or[i] + 0x24));
				DDPDBG("XAGA-OVL[%s] CON0=0x%08x SRC_SIZE0=0x%08x PITCH0=0x%08x ADDR0=0x%08x ADDR_MSB0=0x%08x RDMA_CTRL0=0x%08x\n",
				       oname[i], readl(or[i] + 0x30), readl(or[i] + 0x38),
				       readl(or[i] + 0x44), readl(or[i] + 0xf40),
				       readl(or[i] + 0xf44), readl(or[i] + 0xc0));
			}
			DDPDBG("XAGA-XBAR OVL0_MOUT_EN=0x%08x OVL0_2L_MOUT_EN=0x%08x OVL1_2L_MOUT_EN=0x%08x\n",
			       readl(mx + 0xf08), readl(mx + 0xf04), readl(mx + 0xf0c));
			DDPDBG("XAGA-XBAR MMSYS_OVL_CON=0x%08x OVL0_BLENDOUT_SOUT=0x%08x OVL1_2L_BLENDOUT_SOUT=0x%08x\n",
			       readl(mx + 0xf4c), readl(mx + 0xf6c), readl(mx + 0xfa0));
			DDPDBG("XAGA-XBAR OVL0_2L_OVL1_OVL1_2L_BGOUT=0x%08x TOVL0_OUT1_SEL_IN=0x%08x TOVL0_OUT1_MOUT=0x%08x\n",
			       readl(mx + 0xf68), readl(mx + 0xf70), readl(mx + 0xf74));
			DDPDBG("XAGA-XBAR RDMA0_SEL_IN=0x%08x RDMA0_SOUT_SEL=0x%08x RDMA0_RSZ0_SEL_IN=0x%08x RSZ0_MOUT=0x%08x\n",
			       readl(mx + 0xf34), readl(mx + 0xf38), readl(mx + 0xf3c),
			       readl(mx + 0xf7c));
			DDPDBG("XAGA-XBAR DITHER0_MOUT=0x%08x DSI0_SEL_IN=0x%08x DSC_WRAP_SOUT_SEL=0x%08x\n",
			       readl(mx + 0xf50), readl(mx + 0xf54), readl(mx + 0xf80));
			DDPDBG("XAGA-XBAR RSZ0_SEL_IN=0x%08x RDMA1_SEL_IN=0x%08x RDMA1_SOUT_SEL=0x%08x PQ0_SOUT=0x%08x\n",
				     ...
				     const struct mipi_dsi_msg *msg, u8 flag)
{
	...
	DDPMSG("%s timeout\n", __func__);
	/* v149 probe: this is where jdi_enable's 0xFB write dies with -ETIME on
	 * the warm re-enable path (cold boot succeeds). Dump the DSI engine
	 * state so we can tell what the VM command channel is waiting on.
	 */
	DDPDBG("XAGA-VMCMD timeout: type=0x%02x len=%zu | INTSTA=0x%08x INTEN=0x%08x START=0x%08x CON=0x%08x MODE=0x%08x\n",
	       msg->type, msg->tx_len,
	       readl(dsi->regs + DSI_INTSTA), readl(dsi->regs + DSI_INTEN),
	       readl(dsi->regs + DSI_START), readl(dsi->regs + DSI_CON_CTRL),
	       readl(dsi->regs + DSI_MODE_CTRL));
	DDPDBG("XAGA-VMCMD timeout: STATE160=0x%08x DBG7(164)=0x%08x DBG8(168)=0x%08x DBG9(16c)=0x%08x VACT=0x%08x\n",
	       readl(dsi->regs + 0x160), readl(dsi->regs + 0x164),
	       readl(dsi->regs + 0x168), readl(dsi->regs + 0x16c),
	       readl(dsi->regs + DSI_VACT_NL));
	return -ETIME;
}
				     ...
				     const struct mipi_dsi_msg *msg)
{
	...
	DDPDBG("XAGA-DSI[transfer] type=0x%02x tx_len=%d mode_ctrl=0x%08x irq=0x%02x\n",
	       msg->type, msg->tx_len,
...
static int mtk_dsi_probe(struct platform_device *pdev)
{
	...
	if (!dsi->is_slave) {
		...
		if (endpoint) {
			...
			if (dsi->panel)
				dsi->ext = find_panel_ext(dsi->panel);
			DDPDBG("XAGA-DBG[dsi_probe] panel=%px ext=%px\n",
				dsi->panel, dsi->ext);
			if (dsi->slave_dsi) {
	...
	DDPDBG("XAGA-DSI[probe] INTSTA=0x%08x INTEN=0x%08x START=0x%08x\n",
	       readl(dsi->regs + DSI_INTSTA), readl(dsi->regs + DSI_INTEN),
	...
	/*
	 * Vendor assumes LK already enabled DSI0 and hands the engine over, so it
	 * pre-acquires PHY/clocks here and starts clk_refcnt at 1. This bring-up
	 * parks the LK takeover instead, so that assumption does not hold: the
	 * pre-acquire makes the first mtk_dsi_poweron() bail out at
	 * "++clk_refcnt != 1", which skips mtk_dsi_set_data_rate() and leaves
	 * mipi_tx0_pll at 0 Hz. Without the HS byte clock the hardware refuses to
	 * latch CON_CTRL.DSI_EN, so the VDO engine never produces SOF.
	 *
	 * Let refcount start at 0 and have the first poweron run the full
	 * set_data_rate -> lane_config -> phy_power_on -> engine/digital clk
