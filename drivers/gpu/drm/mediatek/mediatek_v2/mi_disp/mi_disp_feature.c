...
err:
	...
	/* failed attach must not leave half-attached state behind */
	dd_ptr->display = NULL;
...
int mi_disp_feature_detach_display(void *display, int disp_id, int intf_type)
{
	...
	if (is_support_disp_id(disp_id) && is_support_disp_intf_type(intf_type)) {
		...
		/*
		 * Deferred probe / aborted master bind can run unbind
		 * before attach ever completed. Detach must be idempotent
		 * instead of dereferencing never-initialized pointers
		 * (device_unregister(NULL) -> panic at NULL+0x30).
		 */
		if (!dd_ptr->display) {
			DISP_INFO("detach skipped: display %d never attached\n",
				disp_id);
			return 0;
		}
#ifdef CONFIG_MI_DISP_BOOST
...
#ifdef CONFIG_MI_DISP_SYSFS
		if (dd_ptr->dev && !IS_ERR(dd_ptr->dev))
			mi_disp_remove_device_attributes(dd_ptr->dev);
#endif
		if (dd_ptr->dev && !IS_ERR(dd_ptr->dev))
			device_unregister(dd_ptr->dev);
