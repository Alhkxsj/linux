...
static void mtk_dsc_start(struct mtk_ddp_comp *comp, struct cmdq_pkt *handle)
{
	...
	DDPDBG("XAGA-STAGE dsc_start: CON=0x%08x INTSTA=0x%08x\n",
...
static void mtk_dsc_stop(struct mtk_ddp_comp *comp, struct cmdq_pkt *handle)
{
	...
	DDPDBG("XAGA-STAGE dsc_stop: CON=0x%08x\n",
...
//	mtk_dsc_default_setting();
	...
	DDPDBG("XAGA-DSC-PARAMS ver=%d slice_mode=%d rgb_swap=%d dsc_cfg=%d rct=%d bpc=%d lbuf=%d bp=%d bpp=%d\n",
	...
	DDPDBG("XAGA-DSC-PARAMS pic=%dx%d slice=%dx%d chunk=%d xmit=%d dec=%d\n",
	...
	DDPDBG("XAGA-DSC-PARAMS scale=%d incr=%d decr=%d lbpo=%d nfl=%d sbpo=%d init=%d fin=%d\n",
	...
	DDPDBG("XAGA-DSC-PARAMS fmin=%d fmax=%d rc=%d edge=%d q0=%d q1=%d tgt_hi=%d tgt_lo=%d\n",
	...
	if (dsc_params->enable == 1) {
...

		{
			...
			DDPDBG("XAGA-DSC CON=0x%08x MODE=0x%08x ENC_W=0x%08x\n",
				readl(baddr + DISP_REG_DSC_CON),
				readl(baddr + DISP_REG_DSC_MODE),
				readl(baddr + DISP_REG_DSC_ENC_WIDTH));
			DDPDBG("XAGA-DSC PIC_W=0x%08x PIC_H=0x%08x SLICE_W=0x%08x SLICE_H=0x%08x\n",
			...
			DDPDBG("XAGA-DSC CHUNK=0x%08x BUF=0x%08x PPS0=0x%08x PPS1=0x%08x\n",
