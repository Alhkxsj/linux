...
static const struct mtk_mmsys_driver_data mt6895_mmsys_driver_data = {
	...
	/*
	 * PGZ110: shadow_register=true (workaround kept from earlier sessions).
	 * NOTE: reverting this to vendor default (false) was tested (v134) and
	 * RE-INTRODUCED the setbacklight deadlock (Xorg stuck D-state in
	 * mtk_drm_setbacklight, idlemgr-0 D-state, /sys/.../state blocks on
	 * drm_modeset_lock). With false, ddp_config runs only from FRAME_DONE
	 * IRQ which never arrives under this bring-up. Keep true so atomic_flush
	 * drives ddp_config directly (this is what keeps the crtc unblocked).
	 */
	.shadow_register = true,
	.has_smi_limitation = false,
	.doze_ctrl_pmic = true,
...
/*
 * DIAGNOSTIC (v212): hardware takeover on a worker, bounded from probe.
 *
 * Calling mtk_drm_first_enable() straight from mtk_drm_kms_init() (v210,
 * which is exactly what the xaga tree does) hung the device before the USB
 * gadget came up: no SSH, no ACM console, no fastboot, nothing in the
 * pstore ring. With the panel as the only would-be console and ttyGS0
 * banned, a hang in probe context is completely unobservable.
 *
 * v211 pushed the call into a 30 s delayed work, which kept the device
 * alive but was useless: by then the fbdev client had already enabled
 * crtc0, so mtk_drm_crtc_first_enable() took its "already enabled" early
 * return and the takeover never ran at all.
 *
 * So run it on a worker, but have probe wait for it with a timeout. The
 * ordering is now deterministic (the takeover completes before
 * drm_client_setup() creates the fbdev client), while a wedge only wedges
 * the kworker: probe gives up after XAGA_FIRST_ENABLE_TIMEOUT_S, userspace
 * still boots, and the XAGA-FE[nn] markers plus /proc/<kworker>/stack
 * identify the blocking step over SSH. Instrumentation, not the final
 * shape - once the blocker is known, the call belongs inline in kms_init.
 */
#define XAGA_FIRST_ENABLE_TIMEOUT_S	5
...
static struct drm_device *xaga_first_enable_drm;
static DECLARE_COMPLETION(xaga_first_enable_done);
static void xaga_first_enable_workfn(struct work_struct *work)
{
	struct drm_device *drm = xaga_first_enable_drm;
...
static DECLARE_WORK(xaga_first_enable_work, xaga_first_enable_workfn);
...
static int mtk_drm_kms_init(struct drm_device *drm)
{
	...
	/*
	 * GCE readiness gate. We run with fw_devlink=off, so nothing
	 * orders the DRM master after the GCE mailbox controller; without
	 * this gate every deferred-probe trigger re-ran a full
	 * bind_all/abort/unbind_all cycle and flooded the console.
	 * Defer quietly until cmdq_probe has registered the mailbox,
	 * then proceed exactly once.
	 */
	{
		...
		if (!gce_pdev || !gce_pdev->dev.driver) {
			dev_err_ratelimited(drm->dev,
				"XAGA-GATE: gce not probed yet, deferring\n");
			if (gce_pdev)
	...
	/*
	 * Hardware takeover. Runs on a worker so that a hang lands in a
	 * kworker with SSH alive instead of killing probe context
	 * unobservably, but probe waits for it here so the takeover is
	 * ordered before drm_client_setup() enables the CRTC behind our back
	 * (see the comment on mtk_drm_first_enable).
	 */
	xaga_first_enable_drm = drm;
...
static int mtk_drm_bind(struct device *dev)
{
	...
	/*
	 * LK hands the panel over as a live "simple-framebuffer" scanout at
	 * 0xfe1af000, and simplefb keeps driving it (fb0 + fbcon) after the
	 * kernel boots. Without evicting it, simplefb and this driver program
	 * the same OVL/RDMA/DSI hardware with conflicting state, which is
	 * visible as an endless DSI0 buffer-underrun storm and a frozen VDO
	 * engine. Drop the conflicting framebuffer before KMS touches the
	 * hardware.
	 */
	ret = aperture_remove_all_conflicting_devices(mtk_drm_driver.name);
	if (ret)
		DDPPR_ERR("%s: failed to remove conflicting fb: %d\n",
			  __func__, ret);
	else
		DDPINFO("%s: conflicting framebuffers removed\n", __func__);
...
SKIP_SIDE_DISP:
	...
	/* 显示 master 接管起点: LK 可能遗留 MDP_RDMA0/1 处于运行态并持续
	 * 读取 boot logo 旧缓冲, 造成 mdp_iommu 持续 fault。此处按 DTS 资源
	 * 描述取寄存器并清 RDMA_EN 停引擎。不写 RESET: 复位释放会把 EN 弹
	 * 回默认使能态, 反而重新激活旧读(MT6895 实测)。
	 * MDP_RDMA_EN=0x000(bit0), MDP_RDMA_SRC_BASE_0=0xf00, 与
	 * drivers/gpu/drm/mediatek/mtk_mdp_rdma.c 寄存器定义一致。
	 */
{
	static const char *const mdp_rdma_compat[] = {
		"mediatek,mdp_rdma0", "mediatek,mdp_rdma1",
	};
#define MDP_RDMA_EN_OFFSET			0x000
#define MDP_RDMA_SRC_BASE_0_OFFSET		0xf00
	unsigned int r;

	for (r = 0; r < ARRAY_SIZE(mdp_rdma_compat); r++) {
		struct device_node *rdma_node;
		struct resource res;
		void __iomem *regs;

		rdma_node = of_find_compatible_node(NULL, NULL,
						    mdp_rdma_compat[r]);
		if (!rdma_node)
			continue;
		if (of_address_to_resource(rdma_node, 0, &res)) {
		...
		dev_info(dev, "%s takeover @%pR: EN=0x%08x -> 0 (SRC_BASE=0x%08x)\n",
			 mdp_rdma_compat[r], &res,
			 readl_relaxed(regs + MDP_RDMA_EN_OFFSET),
			 readl_relaxed(regs + MDP_RDMA_SRC_BASE_0_OFFSET));
		writel_relaxed(0, regs + MDP_RDMA_EN_OFFSET);
		iounmap(regs);
		of_node_put(rdma_node);
	}
#undef MDP_RDMA_SRC_BASE_0_OFFSET
