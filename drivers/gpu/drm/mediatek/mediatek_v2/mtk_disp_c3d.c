...
static void disp_c3d_sram_write_init_sram(struct mtk_ddp_comp *comp)
{
	...
	if (comp->regs) {
		...
		DDPDBG("XAGA-C3D: %s pre-GCE EN=0x%08x SRAM_CFG=0x%08x SRAM_STATUS=0x%08x IF0=0x%08x IF1=0x%08x\n",
		...
		DDPDBG("XAGA-C3D: %s pre-GCE after CPU write IF0=0x%08x IF1=0x%08x\n",
			dev_name(comp->dev),
...
static void mtk_disp_c3d_prepare(struct mtk_ddp_comp *comp)
{
	...
	if (comp->regs) {
		...
		DDPDBG("XAGA-C3D: %s EN=0x%08x CFG=0x%08x INTSTA=0x%08x SRAM_CFG=0x%08x SRAM_STATUS=0x%08x IF0=0x%08x IF1=0x%08x\n",
		...
		DDPDBG("XAGA-C3D: %s after CPU write IF0=0x%08x IF1=0x%08x\n",
			dev_name(comp->dev),
			readl(comp->regs + C3D_SRAM_RW_IF_0),
			readl(comp->regs + C3D_SRAM_RW_IF_1));
	}
