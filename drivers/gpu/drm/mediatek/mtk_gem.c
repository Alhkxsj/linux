				   ...
				   size_t size, bool alloc_kmap)
{
	...
	DRM_DEBUG_DRIVER("cookie = %p dma_addr = %pad size = %zu\n",
			 ...
			 size);
	pr_info("XAGA-GEM: dma_addr = 0x%llx phys = 0x%llx size = 0x%zx cookie = %p\n",
		(unsigned long long)mtk_gem->dma_addr,
		(unsigned long long)virt_to_phys(mtk_gem->cookie), size,
		mtk_gem->cookie);
