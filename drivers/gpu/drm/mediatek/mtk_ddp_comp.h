...
/*
 * DSC encoder parameters, sourced from the attached DSC panel. The mainline
 * mtk_dsi driver has no DSC support of its own, so the DSC ddp component gets
 * these from the panel (via mtk_dsi) so it can program the DISP_DSC block.
 * Only the fields DISP_DSC uses are kept; the VESA PPS is derived by the DSI
 * host and sent to the panel separately.
 */
struct mtk_dsc_config_params {
	bool enable;
	unsigned int ver;
