...
static const unsigned int mt6895_mtk_ddp_main[] = {
	DDP_COMPONENT_OVL0,
	DDP_COMPONENT_RDMA0,
	DDP_COMPONENT_DSC0,
	DDP_COMPONENT_DSI0,
...
static const struct mtk_drm_route mt6895_mtk_ddp_main_routes[] = {
	{0, DDP_COMPONENT_DSI0},
};
...
static const struct mtk_mmsys_driver_data mt6895_mmsys0_driver_data = {
	.main_path = mt6895_mtk_ddp_main,
	.main_len = ARRAY_SIZE(mt6895_mtk_ddp_main),
	.conn_routes = mt6895_mtk_ddp_main_routes,
	.num_conn_routes = ARRAY_SIZE(mt6895_mtk_ddp_main_routes),
	.shadow_register = true,
	.vblank_comp_index = 1,
	.mmsys_dev_num = 1,
	.max_width = 8191,
...
static const struct of_device_id mtk_drm_of_ids[] = {
	...
	{ .compatible = "mediatek,mt6895-mmsys0",
	  .data = &mt6895_mmsys0_driver_data},
	{ .compatible = "mediatek,mt6895-mmsys1",
	  .data = &mt6895_mmsys1_driver_data},
...
static int mtk_drm_kms_init(struct drm_device *drm)
{
	...
	pr_info("XAGA-STAGE kms_init DONE: drm=%p crtc0=%s fbcon will commit\n",
		drm, crtc ? crtc->name : "none");
...
/*
 * XAGA diag: debugfs hook to READ/WRITE physical memory safely (no /dev/mem,
 * which requires STRICT_DEVMEM off and that made the display grey).
 * Uses memremap() so system RAM (e.g. the fbcon CMA buffer) is accessible.
 *   echo ADDR > /sys/kernel/debug/xaga_mem               -> prints the u32
 *   echo ADDR VAL > /sys/kernel/debug/xaga_mem           -> writes VAL
 *   echo ADDR VAL COUNT > /sys/kernel/debug/xaga_mem     -> fills COUNT u32s
 */
static ssize_t xaga_mem_dbgfs_write(struct file *file, const char __user *ubuf,
				    size_t count, loff_t *pos)
{
	char buf[96];
	unsigned long addr, val, cnt = 1;
	void *p;
	int n, i;
	...
	n = sscanf(buf, "%lx %lx %lx", &addr, &val, &cnt);
	if (n < 1) {
		pr_info("xaga_mem: echo <hex addr> [<hex val> [<count>]] > xaga_mem\n");
		return count;
	}
	if (cnt > 1048576)
		cnt = 1048576;
	p = memremap(addr & ~0xfffUL, ((addr & 0xfff) + cnt * 4 + 0xfff) & ~0xfffUL,
		     MEMREMAP_WB);
	if (!p) {
		pr_info("xaga_mem: memremap failed for 0x%lx\n", addr);
		return count;
	}
	if (n == 1) {
		pr_info("xaga_mem read [0x%lx] = 0x%08x\n", addr,
			*(u32 *)(p + (addr & 0xfff)));
	} else {
		for (i = 0; i < (int)cnt; i++)
			*(u32 *)(p + (addr & 0xfff) + i * 4) = (u32)val;
		pr_info("xaga_mem write [0x%lx] = 0x%08x x%lu\n", addr,
			(u32)val, cnt);
	}
	memunmap(p);
	return count;
...
static const struct file_operations xaga_mem_fops = {
	.write = xaga_mem_dbgfs_write,
};
...
static int mtk_drm_bind(struct device *dev)
{
	...
	/*
	 * XAGA: delay the first REAL display touch (kms_init -> fbcon
	 * commit -> hw_init stop/reset/panel-init) to ~0.85s, matching the
	 * downstream kernel's timing (its DRM probe at 0.776s, kms_init
	 * at 0.916s). LK's video stream needs that time to settle; acting
	 * at ~0.14s disturbs it mid-startup -> random grey handoff.
	 * Driver probing stays at the normal early time (no deferral
	 * storm like a probe-time sleep caused).
	 */
	msleep(700);
	...
	/* XAGA diag: debugfs memory read hook */
	debugfs_create_file("xaga_mem", 0200, NULL, NULL, &xaga_mem_fops);

	/*
	 * XAGA: run the fbcon commit - the kernel must configure the display
	 * (LK leaves a broken state: grey + DSC ABN_EOF). It programs
	 * OVL1_2L (content pipe) at our buffer and OVL0 as relay.
	 */
