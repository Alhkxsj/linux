				     ...
				     struct drm_fb_helper_surface_size *sizes)
{
	...
	drm_dbg_kms(dev, "surface width(%d), height(%d) and bpp(%d)\n",
		    ...
		    sizes->surface_bpp);
	pr_err("XAGA-FBDEV: dma probe enter\n");
	...
	if (IS_ERR(buffer)) {
		pr_err("XAGA-FBDEV: framebuffer_create failed %ld fmt=%08x\n",
			PTR_ERR(buffer), format);
	...
	if (IS_ERR(info)) {
		pr_err("XAGA-FBDEV: alloc_info failed %ld\n", PTR_ERR(info));
		ret = PTR_ERR(info);
	...
	if (ret) {
		pr_err("XAGA-FBDEV: probe tail failed %d\n", ret);
