...
/*
 * WORKAROUND (MT6895): runtime callbacks only, no system-sleep callbacks.
 *
 * DEFINE_RUNTIME_DEV_PM_OPS() would route system sleep through
 * pm_runtime_force_suspend()/_resume(), which ignores the
 * pm_runtime_forbid() done in panthor_probe() and suspends the GPU anyway.
 * On MT6895 the GPU power islands belong to the gpueb firmware and the
 * MFGPLL registers (0x13fa0000) are only accessible while they are on, but
 * panthor_device_resume() calls clk_prepare_enable(clks.core) before anyone
 * asks the EB to power up. mtk_pll_prepare() then reads MFGPLL_CON3 on an
 * unpowered island and takes a synchronous external abort, killing the
 * machine on the first suspend attempt.
 *
 * Keeping the GPU powered across system sleep costs standby power. The real
 * fix is to order mt6895_gpueb_power_on()/_off() around the clock and PLL
 * accesses in panthor_device_resume()/_suspend(); that needs the EB
 * power-off path (mt6895_gpueb_power_control(false)), which currently has no
 * callers and is untested. Drop this override once that lands.
 */
