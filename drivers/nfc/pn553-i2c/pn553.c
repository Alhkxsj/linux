...
/*
 * Local boot-mode lookup (stock carries this in the oplus
 * soc/oplus/system/boot_mode.h + a vendor provider). Only the FACTORY
 * check in probe consumes it; parse "atag,boot" from /chosen the way
 * the vendor LK publishes it, defaulting to normal boot.
 */
struct tag_bootmode {
	u32 size;
	u32 tag;
...
static int pn544_get_boot_mode(void)
{
	struct device_node *chosen;
	const struct tag_bootmode *tag;
	int bootmode = MSM_BOOT_MODE__NORMAL;
	...
	tag = of_get_property(chosen, "atag,boot", NULL);
	if (tag)
		bootmode = le32_to_cpup(&tag->bootmode);
	of_node_put(chosen);
...
/* qqcandy: chipset identity comes from DT (nfc_nxp@28); no oplus project-data dependency */
...
/* exported for eSE consumers (see pn553.h users) */
...
static int pn544_probe(struct i2c_client *client)
{
    ...
    //#ifdef OPLUS_FEATURE_NFC_CONSOFT
    //Add for : ST NXP chip common software
    /* qqcandy: chip support is implied by the DT node (SN100T, NXP PN544-class); */
    //#endif /* OPLUS_FEATURE_NFC_CONSOFT */
...
//#endif /* OPLUS_FEATURE_NFC_BRING_UP */
    ...
    check_hw_info();
#endif
    pr_info("%s:%d get boot mode = %d", __func__, __LINE__, pn544_get_boot_mode());
    if(pn544_get_boot_mode() == MSM_BOOT_MODE__FACTORY){
        pr_info("%s:%d enter ftm mode, set ven = 0", __func__, __LINE__);
        gpio_set_value(pn544_dev->ven_gpio, 0);
    } else {
        pr_info("%s:%d net ftm mode, set ven = 1 defaultly", __func__, __LINE__);
        msleep(5);
        gpio_set_value(pn544_dev->ven_gpio, 1);
    }
