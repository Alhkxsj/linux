...
static int clk_mt6895_ufs_gates_probe(struct platform_device *pdev,
       const struct mtk_gate *gates, int num)
{
struct clk_hw_onecell_data *clk_data;
struct device_node *node = pdev->dev.of_node;
int r;
...
clk_data = mtk_alloc_clk_data(num);
r = mtk_clk_register_gates(&pdev->dev, node, gates, num, clk_data);
if (r)
return r;
...
static int clk_mt6895_ufsao_probe(struct platform_device *pdev)
{
return clk_mt6895_ufs_gates_probe(pdev, ufsao_clks,
  ARRAY_SIZE(ufsao_clks));
}

static int clk_mt6895_ufspdn_probe(struct platform_device *pdev)
{
return clk_mt6895_ufs_gates_probe(pdev, ufspdn_clks,
  ARRAY_SIZE(ufspdn_clks));
}
...
static const struct of_device_id of_match_clk_mt6895[] = {
	{
		...
		.compatible = "mediatek,mt6895-ufscfg-ao",
		.data = clk_mt6895_ufsao_probe,
	}, {
		.compatible = "mediatek,mt6895-ufscfg-pdn",
		.data = clk_mt6895_ufspdn_probe,
	}, {
		/* sentinel */
	}
};
