 ...
 *
 * Polling cannot wake the system though: the delayed work is frozen across
 * suspend, so a "wakeup-source" key would never be noticed and s2idle has
 * no way out.  For those keys we additionally request the PMIC press IRQ
 * supplied by the mtk-spmi-pmic MFD cell and arm it as a wakeup source.
 * Requesting it is what actually sets the per-IRQ enable bit inside the
 * PMIC (see mtk_spmi_pmic_irq_sync_unlock()), which in turn lets the chip
 * assert the shared rcs_irq line; that parent IRQ is already wake-enabled
 * by the MFD.  Key reporting itself stays entirely in the poll loop - the
 * handler only accounts the wakeup event.
 *
 * DT binding:
 *   pmic@4 {
...
static irqreturn_t mtk_keys_wake_irq(int irq, void *data)
{
	struct mtk_keys *keys = data;

	/*
	 * Nested (threaded) PMIC IRQ.  The press is reported by the poll
	 * work as usual once it is unfrozen; all this handler has to do is
	 * account the wakeup so the s2idle loop stops waiting.
	 */
	pm_wakeup_event(keys->dev, 0);

	return IRQ_HANDLED;
}

static void mtk_keys_setup_wakeup(struct mtk_keys *keys,
				  struct platform_device *pdev)
{
	...
	/*
	 * mt6363_keys_resources in mtk-spmi-pmic.c hands us four IRQs in a
	 * fixed order: PWRKEY, HOMEKEY, PWRKEY_R, HOMEKEY_R.  Only the press
	 * IRQ is needed to leave suspend.
	 */
	for (i = 0; i < keys->num_keys; i++) {
		if (!keys->keys[i].wakeup)
			continue;

		keys->keys[i].irq = platform_get_irq_optional(pdev, i);
		if (keys->keys[i].irq < 0) {
			dev_info(dev, "key %d: no PMIC IRQ, cannot wake (%d)\n",
				 i, keys->keys[i].irq);
			continue;
		}

		error = devm_request_threaded_irq(dev, keys->keys[i].irq, NULL,
						 mtk_keys_wake_irq,
						 IRQF_ONESHOT, "mtk-spmi-keys",
						 keys);
		if (error) {
			dev_warn(dev, "key %d: failed to request IRQ %d: %d\n",
				 i, keys->keys[i].irq, error);
			keys->keys[i].irq = -ENXIO;
			continue;
		}
		...
		/*
		 * Keeps the IRQ armed for good: suspend_device_irq() then
		 * leaves it enabled instead of masking it, so the PMIC enable
		 * bit written at request time survives suspend.
		 */
		error = enable_irq_wake(keys->keys[i].irq);
		if (error)
			dev_warn(dev, "key %d: failed to arm IRQ %d as wakeup: %d\n",
				 i, keys->keys[i].irq, error);
		else
			dev_info(dev, "key %d: IRQ %d armed as wakeup source\n",
				 i, keys->keys[i].irq);
	}
}
...
static int mtk_keys_probe(struct platform_device *pdev)
{
	...
	if (keys->num_keys > 0) {
		...
		mtk_keys_setup_wakeup(keys, pdev);
