...
{
	/*
	 * Set the direction rather than just the value: the line loses its output
	 * configuration while the driver still holds the descriptor, so a plain
	 * gpiod_set_value() pulse does not reach the chip.
	 *
	 * The delays are sized for a cold start, not a warm one. A chip that has
	 * dropped off the bus after going idle needs far longer than the 8 ms that
	 * is enough at probe time: with 8 ms the five check_chipid attempts all
	 * fall inside the window where the chip does not answer yet, the driver
	 * gives up, and the half-written SRAM that a failed recovery leaves behind
	 * plays as a raspy, distorted buzz.
	 */
	gpiod_direction_output(aw8697->reset_gpio, 0);
	usleep_range(30000, 31000);
	gpiod_direction_output(aw8697->reset_gpio, 1);
	msleep(100);
	return 0;
}
...
static int aw8697_check_chipid(struct aw8697 *aw8697)
{
	...
	while (cnt < 5) {
		...
		/*
		 * A read error means the chip has not come back yet, so it has to
		 * be retried like a wrong ID would be -- returning here made the
		 * five attempts useless for the one case that needs them, a chip
		 * that dropped off the bus and is slow to answer after a reset.
		 */
		if (!aw8697_i2c_read(aw8697, AW8697_REG_ID, &val) &&
		    val == AW8697_CHIP_ID) {
		...
		if (!aw8697_i2c_read(aw8697, AW86927_REG_IDH, &idh) &&
		    !aw8697_i2c_read(aw8697, AW86927_REG_IDL, &idl) &&
		    ((u16)idh << 8 | idl) == AW86927_CHIP_ID) {
				       ...
				       const u8 *data, int len)
{
	...
	/*
	 * Both the RAMINIT bit and the address pointer have to be in effect for
	 * the read-back to mean anything. When they are not, RAMDATA returns
	 * whatever the pointer happens to sit on -- a smooth ramp out of the
	 * waveform table, which looks like corrupted data rather than a failed
	 * setup. Log the registers so the two cases can be told apart.
	 */
	if (!aw8697_i2c_read(aw8697, AW8697_REG_SYSCTRL, &sysctrl) &&
	    !aw8697_i2c_read(aw8697, AW8697_REG_RAMADDRH, &addrh) &&
	    !aw8697_i2c_read(aw8697, AW8697_REG_RAMADDRL, &addrl))
		dev_info(aw8697->dev,
			"before verify: sysctrl=0x%02x raminit=%d ramaddr=0x%02x%02x\n",
			sysctrl, !!(sysctrl & AW8697_SYSCTRL_RAMINIT_MASK),
			addrh, addrl);
...
/*
 * The controller stops acknowledging I2C tens of seconds after it is reset --
 * measured on qqcandy: alive at probe, gone by t+38 s, and i2cget on an unbound
 * bus fails for every register until the reset line is toggled again. Playback
 * therefore has to assume the chip may have dropped off and bring it back:
 * a hardware reset restores the interface but clears SRAM and every register, so
 * the init sequence and the waveform blob have to be replayed with it.
 *
 * Cost when the chip is present is one register read.
 */
static int aw8697_ensure_alive(struct aw8697 *aw8697)
{
	u8 val;
	int ret;
	...
	dev_info(aw8697->dev, "chip is off the bus, resetting\n");
	...
	if (!aw8697_i2c_read(aw8697, AW8697_REG_GLB_STATE, &val))
		dev_info(aw8697->dev, "after reset: glb_state=0x%02x\n", val);
...
static void aw8697_play_work(struct work_struct *work)
{
	...
	if (aw8697->state && aw8697_ensure_alive(aw8697)) {
		dev_err(aw8697->dev, "cannot reach the controller, skipping\n");
...
static int aw8697_probe(struct i2c_client *client)
{
	...
	/*
	 * Name the device after the chip that was actually detected, following
	 * the "<chip>-haptics" convention the input subsystem uses for these
	 * parts. Userspace matches on this: feedbackd's udev rules key off
	 * ATTRS{name} to tag a device as FEEDBACKD_TYPE=vibra, and a name it does
	 * not know means haptics stay silent even though the device works.
	 */
	input_dev->name = aw8697->chip == AW_CHIP_8697 ? "aw8697-haptics"
						       : "aw86927-haptics";
