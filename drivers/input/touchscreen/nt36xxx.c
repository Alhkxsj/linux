...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	if (!of_property_read_u32_array(np, "touchpanel,panel-coords", coords, 2)) {
		ts->abs_x_max = coords[0];
		ts->abs_y_max = coords[1];
		ts->panel_coords_valid = true;
		NVT_LOG("panel-coords=%u,%u\n", coords[0], coords[1]);
	}
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	} else if (10 == ts->super_resolution_factor) {
