...
static int auxadc_handle_vbat0(struct mt6375_priv *priv, bool is_vbat0)
{
	...
	if (!chg_psy) {
		chg_psy = devm_power_supply_get_by_reference(priv->dev, "charger");
		if (IS_ERR(chg_psy))
			chg_psy = NULL;
	}
	if (chg_psy) {
...
static void vbat0_alarm_poll_func(
...
static int mt6375_auxadc_parse_dt(struct mt6375_priv *priv)
{
	...
	np = of_get_child_by_name(priv->dev->of_node, "imix_r");
	if (!np) {
		dev_notice(priv->dev, "no imix_r\n");
		return -ENODEV;
	}
	...
	of_node_put(np);
	if (ret) {
		dev_notice(priv->dev, "failed to read imix_r val (%d)\n", ret);
		return ret;
	}
	priv->imix_r = val;
...
static int mt6375_auxadc_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(priv->isink_load)) {
		dev_err(&pdev->dev, "Failed to get isink_load regulator [%ld]\n",
