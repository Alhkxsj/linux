...
int mtk_build_eint(struct mtk_pinctrl *hw, struct platform_device *pdev)
{
	...
	pr_info("XAGA-EINT: mtk_build_eint enter (%s), EINT_MTK=%d\n",
		dev_name(&pdev->dev), IS_ENABLED(CONFIG_EINT_MTK));

	if (!IS_ENABLED(CONFIG_EINT_MTK))
		return 0;

	if (!of_property_read_bool(np, "interrupt-controller")) {
		pr_info("XAGA-EINT: no interrupt-controller prop\n");
		return -ENODEV;
	}
	...
	if (!eint_node) {
		...
		pr_info("XAGA-EINT: using mediatek,eint phandle node %pOF (%d banks)\n",
			eint_node, count_reg_names);
	}
	hw->eint->nbase = count_reg_names;
	pr_info("XAGA-EINT: nbase=%d\n", hw->eint->nbase);
	if (hw->eint->nbase <= 0) {
		if (eint_node)
			of_node_put(eint_node);
		ret = -EINVAL;
	...
	if (eint_node) {
		...
		{
			/* debug the parse on the apirq node */
			struct of_phandle_args oirq;
			int r = of_irq_parse_one(eint_node, 0, &oirq);
			pr_info("XAGA-EINT: apirq parse r=%d np=%pOF nargs=%d "
				"a0=%d a1=%d a2=%d a3=%d\n",
				r, oirq.np, oirq.args_count,
				oirq.args[0], oirq.args[1], oirq.args[2],
				oirq.args[3]);
			pr_info("XAGA-EINT: apirq node name=%s\n",
				eint_node->name);
		}
	} else {
	...
	pr_info("XAGA-EINT: irq_of_parse_and_map -> %d\n", hw->eint->irq);
	if (!hw->eint->irq) {
		ret = -EINVAL;
		goto err_free_eint;
	}
	...
	if (!hw->soc->eint_hw) {
		pr_info("XAGA-EINT: no eint_hw in soc data\n");
		ret = -ENODEV;
		goto err_free_eint;
	}
	...
	pr_info("XAGA-EINT: mtk_eint_do_init -> %d\n", ret);
	if (ret)
		goto err_free_eint;
