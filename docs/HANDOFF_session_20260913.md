...
> ⚠ **编号与状态说明（2026-09-13 核验）**：本文标题"§80.12 Tag 实际读取验证成功"与 HANDOFF.md 的 §80.12（该轮明文未刷写未读取）**编号撞车且结论相反**。本书记录的读取属实（`logs/ccci_probe_success_20260913_033408.log` 已核验：vermagic 不匹配被拒 → 重编 → 加载 → 27 tag → SMEM 0x8e000000 total=0x120000 / CCB 0x89000000+0x4000000），时间线是：v353 实际于当日 01:07–01:18 已刷双槽重启（见 HANDOFF §80.12 更正框），03:34 由 §80.10 探针的 v353 配套副本（/root/ccci_probe_new.ko，SHA256 59c6f463…）完成读取。27 tag 的权威记录现为 HANDOFF §80.14（ccci_core 路径复验）。git 状态段已过时（现 HEAD 2e715432377f、领先远端 4 提交、stash 仅 1 个旧的）。
